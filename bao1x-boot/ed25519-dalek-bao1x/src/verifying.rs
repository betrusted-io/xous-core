...
impl PartialEq<VerifyingKey> for VerifyingKey {
    fn eq(&self, other: &VerifyingKey) -> bool { self.as_bytes() == other.as_bytes() }
}
...
impl From<&SigningKey> for VerifyingKey {
    fn from(signing_key: &SigningKey) -> VerifyingKey { signing_key.verifying_key() }
}

impl From<EdwardsPoint> for VerifyingKey {
    fn from(point: EdwardsPoint) -> VerifyingKey { VerifyingKey { point, compressed: point.compress() } }
}

impl VerifyingKey {
    /// Convert this public key to a byte array.
    #[inline]
    pub fn to_bytes(&self) -> [u8; PUBLIC_KEY_LENGTH] { self.compressed.to_bytes() }

    /// View this public key as a byte array.
    #[inline]
    pub fn as_bytes(&self) -> &[u8; PUBLIC_KEY_LENGTH] { &(self.compressed).0 }
    ...
    pub fn from_bytes(bytes: &[u8; PUBLIC_KEY_LENGTH]) -> Result<VerifyingKey, SignatureError> {
        ...
        let point = compressed.decompress().ok_or(InternalError::PointDecompression)?;
    ...
    pub fn is_weak(&self) -> bool { self.point.is_small_order() }
        ...
        CtxDigest: Digest<OutputSize = U64>,
    {
        ...
        if expected_R == signature.R { Ok(()) } else { Err(InternalError::Verify.into()) }
    }
    ...
    {
        // crate::println!("raw_verif_prehash");
        let signature = InternalSignature::try_from(signature)?;
        ...
        // crate::println!("msg: {:x?}", message.as_slice());

        let expected_R = RCompute::<CtxDigest>::compute(self, signature, Some(ctx), &message);
        // crate::println!("expected_R: {:x?}, signature.R: {:x?}", expected_R, signature.R);
    ...
    /// # Inputs
    ///
    /// * `prehashed_message` is an instantiated hash digest with 512-bits of output which has had the message
    ///   to be signed previously fed into its state.
    /// * `context` is an optional context string, up to 255 bytes inclusive, which may be used to provide
    ///   additional domain separation.  If not set, this will default to an empty string.
    ...
    pub fn verify_strict(
        ...
        let signature_R =
            signature.R.decompress().ok_or_else(|| SignatureError::from(InternalError::Verify))?;
    ...
    #[cfg(feature = "hazmat")]
    pub fn verify_stream(&self, signature: &ed25519::Signature) -> Result<StreamVerifier, SignatureError> {
        let signature = InternalSignature::try_from(signature)?;
    ...
    pub fn to_montgomery(&self) -> MontgomeryPoint { self.point.to_montgomery() }

    /// Return this verifying key in Edwards form.
    pub fn to_edwards(&self) -> EdwardsPoint { self.point }
}
    ...
    CtxDigest: Digest<OutputSize = U64>,
{
    ...
    pub(crate) fn new(key: &VerifyingKey, signature: InternalSignature, prehash_ctx: Option<&[u8]>) -> Self {
        ...
        h.update(R.as_bytes());
        h.update(A.as_bytes());
        Self { key: *key, signature, h }
    }

    pub(crate) fn update(&mut self, m: &[u8]) { self.h.update(m) }
    ...
    pub(crate) fn finish(self) -> CompressedEdwardsY {
        ...
        EdwardsPoint::vartime_double_scalar_mul_basepoint(&k, &(minus_A), &self.signature.s).compress()
    }
}
    ...
    MsgDigest: Digest<OutputSize = U64>,
{
    fn verify_digest(
        ...
        self.key().verify_prehashed(msg_digest, Some(self.value()), signature)
    }
}
...
impl TryFrom<&[u8]> for VerifyingKey {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        let bytes = bytes
            .try_into()
            .map_err(|_| InternalError::BytesLength { name: "VerifyingKey", length: PUBLIC_KEY_LENGTH })?;
        Self::from_bytes(bytes)
    }
}

impl From<VerifyingKey> for EdwardsPoint {
    fn from(vk: VerifyingKey) -> EdwardsPoint { vk.point }
}
        ...
        D: Deserializer<'d>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for VerifyingKeyVisitor {
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                if remaining > 0 {
                    return Err(serde::de::Error::invalid_length(32 + remaining, &"expected 32 bytes"));
                }
