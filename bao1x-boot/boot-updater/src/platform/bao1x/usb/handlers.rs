...
use super::*;
use crate::{IS_BAOSEC, WRITTEN_BYTES, udc_pointer_check};
...
fn fill_sparse_data(dest: &mut [u8], offset: usize) {
    ...
    for &(addr, block) in super::fat32_base::SPARSE_FILE_DATA.iter() {
        ...
        // patch the volume name so we can tell alt-boot apart. Coded to tolerate short/partial reads.
...
pub fn get_descriptor_request(this: &mut CorigineUsb, value: u16, _index: usize, length: usize) {
    ...
    match (value >> 8) as u8 {
        USB_DT_DEVICE => {
            let mut dd = DeviceDescriptor::default_mass_storage();
                ...
                if let Some(record) = uf2_data {
                        ...
                        && record.address() as usize + record.data().len() <= END_RANGE
                    {
                        ...
                        match rram.write_slice(offset, record.data()) {
                            Err(e) => crate::println!("Write error {:?} @ {:x}", e, offset),
                            Ok(_) => (),
                        };

                        // do some bookkeeping for the UI
                        WRITTEN_BYTES.fetch_add(record.data().len() as u32, Ordering::SeqCst);
                        ...
                        */
                        ...
                        crate::println!("Invalid write address {:x}, block ignored!", record.address());
                    }

                    let status = WRITTEN_BYTES.load(Ordering::SeqCst);
                    ...
                    if status != 0 && status % UX_UPDATE_INTERVAL_BYTES == 0 {
                        if IS_BAOSEC.load(Ordering::SeqCst) {
                            ...
                            let msg = alloc::format!("boot1 - {}k", status / 1024);
                            crate::println!("boot1 - {}k", status / 1024);
