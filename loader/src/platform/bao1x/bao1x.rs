...
#[cfg(feature = "qr")]
use crate::platform::bao1x::{homography, qr};
...
pub fn early_init_hw() -> u32 {
    ...
    // on the bao1x-hal crate to be functional.
    ...
    */
    ...
    #[cfg(any(feature = "board-baosec", feature = "board-baosor"))]
    {
        ...
        if let Some(mut pmic) = pmic {
            ...
            pmic.set_dcdc(&mut i2c, Some((0.9, false)), bao1x_hal::axp2101::WhichDcDc::Dcdc2).unwrap();
            ...
            /*
            pmic.set_ldo(&mut i2c, Some(2.7), bao1x_hal::axp2101::WhichLdo::Aldo2).unwrap();
            pmic.set_dcdc(&mut i2c, Some((1.8, false)), bao1x_hal::axp2101::WhichDcDc::Dcdc4).unwrap();
            pmic.set_dcdc(&mut i2c, Some((1.8, false)), bao1x_hal::axp2101::WhichDcDc::Dcdc5).unwrap();
            ...
            */
            ...
            pmic.set_ldo(&mut i2c, None, bao1x_hal::axp2101::WhichLdo::Aldo3).unwrap();
            ...
            */
            ...
            if false {
                ...
                pmic.set_ldo(&mut i2c, Some(0.9), bao1x_hal::axp2101::WhichLdo::Aldo3).ok();
    ...
    #[cfg(any(feature = "board-baosec", feature = "board-baosor"))]
    {
        ...
        sh1107.init();
        crate::platform::bao1x::bootlogo::show_logo(&mut sh1107);
        sh1107.draw();
        ...
        {
            use bao1x_api::iox::IoxValue;
            use bao1x_hal::sh1107::Mono;
            use bao1x_hal::udma::Udma;
            use ux_api::minigfx::Point;

            use crate::platform::bao1x::gfx;
            use crate::platform::bao1x::qr;
            ...
            let cam_ifram = unsafe {
                bao1x_hal::ifram::IframRange::from_raw_parts(
                    bao1x_hal::board::CAM_IFRAM_ADDR,
                    bao1x_hal::board::CAM_IFRAM_ADDR,
                    bao1x_hal::board::CAM_IFRAM_LEN_PAGES * 4096,
                )
            };
            // this is safe because we turned on the clocks before calling it
            let mut cam = unsafe { bao1x_hal::gc2145::Gc2145::new_with_ifram(cam_ifram) };
            ...
            loop {
                ...
                if candidates_found == 3 {
                    let maybe_qr_corners = QrCorners::from_finders(
                        ...
                        (finder_width
                            + (crate::platform::bao1x::qr::FINDER_SEARCH_MARGIN * finder_width)
                                / (1 + 1 + 3 + 1 + 1)) as usize,
                    );
                    ...
                    let (src, dst) =
                        qr_corners.mapping(&mut il, crate::platform::bao1x::qr::HOMOGRAPHY_MARGIN);
                    for s in src.iter() {
                        if let Some(p) = s {
                            crate::println!("src {:?}", p);
                    ...
                    if all_found {
                        use crate::platform::bao1x::homography::*;
    ...
    #[cfg(feature = "board-bringup")]
    {
        use bao1x_hal::{iox::IoxValue, minigfx::ColorNative, sh1107::Mono, udma::Udma};
        ...
        {
            crate::platform::bao1x::usb::init_usb();
            // this does not return if USB is initialized correctly...
            unsafe {
                crate::platform::bao1x::usb::test_usb();
            }
        }
    }
    ...
    #[cfg(feature = "board-bringup")]
    {
        ...
        let mut trng = bao1x_hal::sce::trng::Trng::new(HW_TRNG_BASE);
        ...
        #[cfg(feature = "quantum-timer-test")]
        {
            ...
            // iox.set_gpio_dir(bao1x_hal::iox::IoxPort::PB, 15, bao1x_hal::iox::IoxDir::Output);
            ...
            iox.set_bio_bit_from_port_and_pin(bao1x_hal::iox::IoxPort::PC, 2).unwrap();
            iox.set_bio_bit_from_port_and_pin(bao1x_hal::iox::IoxPort::PC, 1).unwrap();
            let pin = iox.set_bio_bit_from_port_and_pin(bao1x_hal::iox::IoxPort::PC, 0).unwrap();
            let pin = 0;
    ...
    #[cfg(feature = "usb-test")]
    {
        ...
        let csr = bao1x_hal::usb::compat::AtomicCsr::new(bao1x_hal::usb::utra::CORIGINE_USB_BASE as *mut u32);
        let irq_csr =
            bao1x_hal::usb::compat::AtomicCsr::new(utralib::utra::irqarray1::HW_IRQARRAY1_BASE as *mut u32);
        ...
        let mut usb = unsafe {
            bao1x_hal::usb::driver::CorigineUsb::new(
                ...
                0, // is dummy in no-std
                bao1x_hal::usb::driver::CRG_UDC_MEMBASE,
                csr,
                irq_csr,
            )
        };
        ...
        let mut last_sc = 0;
        loop {
            let next_sc = csr.r(bao1x_hal::usb::utra::PORTSC);
            if last_sc != next_sc {
                last_sc = next_sc;
                crate::println!("**** SC update {:x?}", bao1x_hal::usb::driver::PortSc(next_sc));
                /*
                if bao1x_hal::usb::driver::PortSc(next_sc).pr() {
                    crate::println!("  >>reset<<");
                    usb.start();
                    in_u0 = false;
                    vbus_on_count = 0;
            ...
            if event == bao1x_hal::usb::driver::CrgEvent::None {
                idle_timer += 1;
            } else {
            ...
            if !vbus_on && vbus_on_count == 4 {
                ...
                crate::println!("irq1: {:x}, status: {:x}", irq1, csr.r(bao1x_hal::usb::utra::USBSTS));
                ...
                // usb.print_status(usb.csr.r(bao1x_hal::usb::utra::PORTSC));
                crate::println!("*Enter U0");
                in_u0 = true;
                let irq1 = irq_csr.r(utralib::utra::irqarray1::EV_PENDING);
                // usb.print_status(csr.r(bao1x_hal::usb::utra::PORTSC));
                irq_csr.wo(utralib::utra::irqarray1::EV_PENDING, irq1);
            }
        }
    }
...
#[cfg(feature = "clock-tests")]
unsafe fn clock_tests(udma_uart: &mut bao1x_hal::udma::Uart) {
...
pub fn irq_setup() {
    ...
    let mut uart_irq = bao1x_hal::udma::UartIrq::new();
