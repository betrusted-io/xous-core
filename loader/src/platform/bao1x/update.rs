...
use crate::platform::bao1x::sha512_digest::Sha512Prehash;
use crate::platform::bao1x::usb::{self, SliceCursor, USB_CONNECTED, disable_all_irqs, flush_tx_wrapper};
...
pub fn process_update(perclk: u32) {
    ...
    crate::platform::bao1x::verifier::lifecycle_lock_root();
    ...
    let (rows, cols) = bao1x_hal::board::baosec::setup_kb_pins(&iox_kbd);
                ...
                loop {
                    ...
                    if USB_CONNECTED.load(core::sync::atomic::Ordering::SeqCst) {
                        ...
                        critical_section::with(|cs| {
                            let mut queue =
                                crate::platform::bao1x::usb::driver::USB_RX.borrow(cs).borrow_mut();
                            while let Some(byte) = queue.pop_front() {
                                critical_section::with(|cstx| {
                                    let mut tx =
                                        crate::platform::bao1x::usb::driver::USB_TX.borrow(cstx).borrow_mut();
                                    tx.push_back(byte);
                                });
                            }
                        });
