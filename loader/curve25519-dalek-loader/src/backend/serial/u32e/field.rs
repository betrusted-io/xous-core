...
pub(crate) fn engine(a: &[u8; 32], b: &[u8; 32], op: EngineOp) -> Engine25519 {
    ...
    let mcode: &'static mut [u32] =
        unsafe { core::slice::from_raw_parts_mut(utralib::HW_ENGINE_MEM as *mut u32, 1024) };
    // allocate the first three registers
    let rf: [&'static mut [u32]; 3] = unsafe {
        [
            core::slice::from_raw_parts_mut(
                (utralib::HW_ENGINE_MEM + 0x1_0000 + 0 * 32) as *mut u32,
                8,
            ),
            core::slice::from_raw_parts_mut(
                (utralib::HW_ENGINE_MEM + 0x1_0000 + 1 * 32) as *mut u32,
                8,
            ),
            core::slice::from_raw_parts_mut(
                (utralib::HW_ENGINE_MEM + 0x1_0000 + 2 * 32) as *mut u32,
                8,
            ),
        ]
    };
