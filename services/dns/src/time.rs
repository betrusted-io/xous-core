            ...
            loop {
                ...
                match FromPrimitive::from_usize(msg.body.id()) {
                    ...
                    Some(PrivTimeOp::SusRes) => xous::msg_scalar_unpack!(msg, token, _, _, _, {
                        ...
                        // resync time on resume, but give a little time for other processes to clear as this
                        // is not urgent
        ...
        move || {
            ...
            loop {
                ...
                match op {
                    ...
                    Some(TimeOp::GetUtcTimeMs) => xous::msg_blocking_scalar_unpack!(msg, _, _, _, _, {
                        let t = start_rtc_secs as i64 * 1000i64 + (tt.elapsed_ms() - start_tt_ms) as i64;
            ...
            loop {
                ...
                match opcode {
                    ...
                    Some(TimeOp::HwSync) => match llio.get_rtc_secs() {
                        Ok(val) => {
                            start_rtc_secs = val;
                            start_tt_ms = tt.elapsed_ms();
                        }
                    ...
                    Some(TimeOp::SetUtcTimeMs) => {
                        xous::msg_scalar_unpack!(msg, utc_hi_ms, utc_lo_ms, _, _, {
...
#[allow(dead_code)]
fn to_binary(bcd: u8) -> u8 { (bcd & 0xf) + ((bcd >> 4) * 10) }
#[allow(dead_code)]
...
pub(crate) fn start_time_ux() {
    thread::spawn({
        move || {
            ...
            let timeserver_cid =
                xous::connect(xous::SID::from_bytes(crate::time::TIME_SERVER_PUBLIC).unwrap()).unwrap();
            ...
            loop {
                ...
                match FromPrimitive::from_usize(msg.body.id()) {
                    Some(crate::TimeUxOp::SetTime) => xous::msg_scalar_unpack!(msg, _, _, _, _, {
                        if !pddb_poller.is_mounted_nonblocking() {
                            modals
                                .show_notification(t!("stats.please_mount", locales::LANG), None)
                        ...
                        let date = modals
                            .alert_builder(t!("rtc.set_time_modal", locales::LANG))
                            .field(
                                Some(String::from(t!("rtc.month", locales::LANG))),
                                Some(rtc_ux_validate_month),
                            )
                            .field(
                                Some(String::from(t!("rtc.day", locales::LANG))),
                                Some(rtc_ux_validate_day),
                            )
                            .field(
                                Some(String::from(t!("rtc.year", locales::LANG))),
                                Some(rtc_ux_validate_year),
                            )
                            .field(
                                Some(String::from(t!("rtc.hour", locales::LANG))),
                                Some(rtc_ux_validate_hour),
                            )
                            .field(
                                Some(String::from(t!("rtc.minute", locales::LANG))),
                                Some(rtc_ux_validate_minute),
                            )
                            .field(
                                Some(String::from(t!("rtc.seconds", locales::LANG))),
                                Some(rtc_ux_validate_seconds),
                            )
                            .build()
                        ...
                        for (index, elem) in date.content().iter().enumerate() {
                            let elem =
                                elem.as_str().parse::<u8>().expect("pre-validated input failed to re-parse!");
                            match index {
                                0 => months = elem,
                                1 => days = elem,
                                2 => years = elem,
