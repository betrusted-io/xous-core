...
fn susres_service() {
    ...
    std::thread::spawn({
        ...
        move || {
            loop {
                ...
                while (std::time::Instant::now().checked_duration_since(start).unwrap().as_secs() as u32)
                    < timeout
                {
    ...
    loop {
        ...
        match opcode {
            ...
            Some(Opcode::SuspendRequest) => msg_blocking_scalar_unpack!(msg, _deepsleep, 0, 0, 0, {
