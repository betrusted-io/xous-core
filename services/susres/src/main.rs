...
pub fn timeout_thread(sid0: usize, sid1: usize, sid2: usize, sid3: usize) {
    let sid = xous::SID::from_u32(sid0, sid1, sid2, sid3);
    let ticktimer = ticktimer_server::Ticktimer::new().unwrap();
    loop {
        let msg = xous::receive_message(sid).unwrap();
        match FromPrimitive::from_usize(msg.body.id()) {
            TimeoutOpcode::Run => {
                ticktimer.sleep_ms(TIMEOUT_TIME.load(Ordering::Relaxed) as u64).unwrap();
...
pub fn execution_gate() {
    ...
    loop {
        let msg = xous::recieve_message(execgate_sid).unwrap();
        match FromPrimitive::from_usize(msg.body.id()) {
            // the entire purpose of SupendingNow is to block the thread that sent the message, until we're ready to resume.
            ExecGateOpcode::SuspendingNow => msg_blocking_scalar_unpack!(msg, _, _, _, _, {
                while !SHOULD_RESUME.load(Ordering::Relaxed) {
                    xous::yield_slice();
                }
            }),
...
fn xmain() -> ! {
    ...
    xous::create_thread_4(timeout_thread, sid0, sid1, sid2, sid3).expect("couldn't create timeout thread");
    ...
    loop {
        ...
        match FromPrimitive::from_usize(msg.body.id()) {
            ...
            Opcode::SuspendReady => msg_scalar_unpack!(msg, token, _, _, _, {
                ...
                if let Some(scb) = suspend_subscribers[token] {
                    ...
                    if all_ready {
                        ...
                        //// QUESTION: should we interlock the exec thread until after the do_resume() hardware callback is done? i think so...
