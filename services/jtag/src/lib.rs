...
impl Jtag {
    pub fn new(xns: &xous_names::XousNames) -> Result<Self, xous::Error> {
        ...
        let conn = xns.request_connection_blocking(api::SERVER_NAME_JTAG).expect("Can't connect to JTAG server");
        Ok(Jtag {
            conn
        })
    }
    ...
    pub fn get_id(&self) -> Result<u32, xous::Error> {
        ...
        if let xous::Result::Scalar1(id)= response {
            Ok(id as u32)
        } else {
    ...
    fn mod_ctl_efuse(&self, bits_to_set: u8) -> Result<bool, xous::Error> {
        let erec = self.efuse_fetch().expect("couldn't read efuse record");
        let response = send_message(self.conn,
            Message::new_blocking_scalar(Opcode::EfuseCtlBurn.to_usize().unwrap(), (erec.cntl | bits_to_set) as usize, 0, 0, 0)
    ...
    pub fn burn_efuse_only_boot(&self) -> Result<bool, xous::Error> {
        self.mod_ctl_efuse(0x1)
    }
    pub fn burn_disable_jtag(&self) -> Result<bool, xous::Error> {
        self.mod_ctl_efuse(0x2)
    }
    pub fn burn_efuse_write_protect(&self) -> Result<bool, xous::Error> {
        self.mod_ctl_efuse(0x4)
    }
    pub fn burn_key_access_protect(&self) -> Result<bool, xous::Error> {
        self.mod_ctl_efuse(0x8)
    }
    pub fn burn_user_and_key_access_protect(&self) -> Result<bool, xous::Error> {
        self.mod_ctl_efuse(0x10)
    }
