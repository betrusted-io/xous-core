...
pub(crate) fn ecupdate_thread(sid: xous::SID) {
    ...
    if com.flash_acquire().unwrap() == false {
        log::error!(
            "couldn't acquire exclusive access to the EC updater mechanism. All other operations will fail!"
        );
    }
    ...
    .expect("couldn't map EC firmware package memory range");
    #[cfg(not(target_os = "xous"))]
    for d in unsafe { ec_package.as_slice_mut::<u8>().iter_mut() } {
        *d = 0xFF
    } // simulate blank flash
    #[cfg(not(target_os = "xous"))]
    ...
    .expect("couldn't map EC wf200 package memory range");
    #[cfg(not(target_os = "xous"))]
    for d in unsafe { wf_package.as_slice_mut::<u8>().iter_mut() } {
        *d = 0xFF
    }
        ...
        match FromPrimitive::from_usize(msg.body.id()) {
            ...
            Some(UpdateOp::UpdateGateware) => {
                ...
                if !validate_package(package, PackageType::Ec) {
                    ...
                    modals
                        .show_notification(&format!("{} gateware", t!("ecup.invalid", locales::LANG)), None)
                        .unwrap();
                } else {
            ...
            Some(UpdateOp::UpdateFirmware) => {
                ...
                if !validate_package(package, PackageType::Ec) {
                    log::error!("firmware package did not pass validation");
                    modals
                        .show_notification(&format!("{} firmware", t!("ecup.invalid", locales::LANG)), None)
                        .unwrap();
                } else {
                    let length = u32::from_le_bytes(package[0x28..0x2c].try_into().unwrap());
                    if length == 0xffff_ffff {
                        // nothing was staged at all
                        xous::return_scalar(msg.sender, UpdateResult::PackageInvalid.to_usize().unwrap())
                            .unwrap();
                        continue;
                    }
                    ...
                    if !do_update(
                        ...
                        &modals,
                        package,
                        EC_GATEWARE_LEN + CTRL_PAGE_LEN,
                        EC_FIRMWARE_BASE,
                        length - (EC_GATEWARE_LEN),
                        "firmware",
                    ) {
                        xous::return_scalar(msg.sender, UpdateResult::Abort.to_usize().unwrap()).unwrap();
                        continue;
            ...
            Some(UpdateOp::UpdateWf200) => {
                ...
                if validate_package(package, PackageType::Wf200) {
                    ...
                    modals
                        .show_notification(&format!("{} WF200", t!("ecup.invalid", locales::LANG)), None)
                        .unwrap();
                    xous::return_scalar(msg.sender, UpdateResult::PackageInvalid.to_usize().unwrap())
                        .unwrap();
                    continue;
            ...
            Some(UpdateOp::CheckCompat) => msg_blocking_scalar_unpack!(msg, _, _, _, _, {
                ...
                if ec_rev < net::MIN_EC_REV {
                    ...
                    note.push_str(&format!(
                        "\n\n{}{}",
                        t!("net.ec_current_rev", locales::LANG),
                        ec_rev.to_string()
                    ));
            ...
            Some(UpdateOp::UpdateAuto) => msg_blocking_scalar_unpack!(msg, force_arg, _, _, _, {
                ...
                const MIN_EC_VER_WITH_HASHES: SemVer =
                    SemVer { maj: 0, min: 9, rev: 8, extra: 8, commit: None };
                ...
                if !is_ec_rev_sane(&ec_reported_rev) && !force {
                    ...
                    if retries > RETRY_TOTAL_DURATION_MS {
                        // this is typically a result of the EC itself being bricked...nothing we can do about
                        // it at this point.
                        log::error!(
                            "EC rev report seems bogus: {:?}; aborting autoupdate process.",
                            ec_reported_rev
                        );
                ...
                if length > xous::EC_FW_PKG_LEN {
                    ...
                    if force {
                        modals
                            .show_notification(
                                &format!("{} gateware", t!("ecup.invalid", locales::LANG)),
                                None,
                            )
                            .unwrap();
                    }
                ...
                let semver_bytes = &package[0x1000 + length as usize - SEMVER_OFFSET
                    ..0x1000 + length as usize - SEMVER_OFFSET + SEMVER_LEN];
                ...
                if (pkg_ver > ec_reported_rev) || force {
                    if validate_package(package, PackageType::Ec) {
                        ...
                        let gw_hash = &package[0x1000 + length as usize - GW_HASH_OFFSET
                            ..0x1000 + length as usize - GW_HASH_OFFSET + HASH_LEN];
                        let fw_hash = &package[0x1000 + length as usize - FW_HASH_OFFSET
                            ..0x1000 + length as usize - FW_HASH_OFFSET + HASH_LEN];
                        did_something = true;
                        let mut do_gw = false;
                        let mut do_fw = false;
                        if ec_reported_rev >= MIN_EC_VER_WITH_HASHES {
                            ...
                            match com.flash_verify(
                                EC_FLASH_BASE + EC_GATEWARE_BASE + (length - GW_HASH_OFFSET as u32),
                                &mut ver_ec_raw,
                            ) {
                                Err(_) => {
                                    xous::return_scalar(msg.sender, UpdateResult::Abort.to_usize().unwrap())
                                        .unwrap();
                                    continue;
                                }
                                _ => {}
                    ...
                    log::info!(
                        "EC Autoupdate check found that EC rev {} is newer or same as update rev {}; no update done",
                        ec_reported_rev.to_string(),
                        pkg_ver.to_string()
                    );
                }
                ...
                // *different*, we don't know if it's newer we assume if it's different, we
                // meant to update it.
                if (ec_reported_rev >= MIN_EC_VER_WITH_HASHES) && !force {
                    let mut ver_wf200_raw = [0u8; 256];
                    match com.flash_verify(EC_FLASH_BASE + WF200_FIRMWARE_BASE, &mut ver_wf200_raw) {
                        Err(_) => {
                            xous::return_scalar(msg.sender, UpdateResult::Abort.to_usize().unwrap()).unwrap();
                            continue;
                        }
                    ...
                    // do a shallow check to see if the firmware magic word 'KEYS' is in the location. The
                    // intention is that we would not even try to run the update if
                    // nothing valid has been staged and the location would read as 0xFFFFFFFF.
                    if package[CTRL_PAGE_LEN as usize..CTRL_PAGE_LEN as usize + 4] == [0x4b, 0x45, 0x59, 0x53]
                    {
                        if package[CTRL_PAGE_LEN as usize..CTRL_PAGE_LEN as usize + WF200_HASH_LEN]
                            != ver_wf200_raw[..WF200_HASH_LEN]
                        {
                            run_wf200_update = true;
                            log::info!("wf200 rev is different");
                            log::info!(
                                "package : {:x?}",
                                &package[CTRL_PAGE_LEN as usize..CTRL_PAGE_LEN as usize + WF200_HASH_LEN]
                            );
...
fn do_update(
    ...
    modals.dynamic_notification(Some(&format!("{}", t!("ecup.preparing", locales::LANG))), None).unwrap();
    tt.sleep_ms(3000).ok();
    ...
    if com.flash_erase(flash_start, image_len).unwrap() {
        ...
        modals.dynamic_notification_close().unwrap();
        modals.show_notification(&format!("{}\n({})", t!("ecup.abort", locales::LANG), name), None).unwrap();
        return false;
    }
    xous::yield_slice();
    ...
    for chunks in exact_chunks {
        ...
        if com.flash_program(prog_addr, pages).unwrap() == false {
            ...
            modals
                .show_notification(&format!("{} {}...", t!("ecup.abort", locales::LANG), name), None)
                .unwrap();
            return false;
        }
        prog_addr += 1024;
    ...
    if lessthan_1k.len() > 0 {
        ...
        // take the remainder that's less than 256-bytes, pad it to 256 bytes, and stick it in the very last
        // page
        if leftovers.len() > 0 {
...
fn is_ec_rev_sane(ec_reported_rev: &SemVer) -> bool {
    if ec_reported_rev.maj == 0 && ec_reported_rev.min == 0 && ec_reported_rev.rev == 0 {
        ...
        if ec_reported_rev.maj > 64 {
            ...
            // codes. We're unlikely to ever get to 64 major releases of this code, so if we see a major
            // release number bigger than 64, we flag it. otoh, if we get to the point of the 65th
            // major release...hopefully this code won't still be around anymore.
            false
        } else {
            true
        }
