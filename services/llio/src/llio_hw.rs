...
pub struct Llio {
    ...
    destruct_armed: bool,
    uartmux_cache: u32, /* stash a value of the uartmux -- restore from override into kernel so we can
                         * record KPs on resume */
}
...
fn handle_event_irq(_irq_no: usize, arg: *mut usize) {
    ...
    if xl.event_csr.rf(utra::btevents::EV_PENDING_COM_INT) != 0 {
        if let Some(conn) = xl.handler_conn {
            xous::try_send_message(
                conn,
                xous::Message::new_scalar(Opcode::EventComHappened.to_usize().unwrap(), 0, 0, 0, 0),
            )
            .map(|_| ())
            .unwrap();
        } else {
    ...
    xl.event_csr.wo(utra::btevents::EV_PENDING, xl.event_csr.r(utra::btevents::EV_PENDING));
}
fn handle_gpio_irq(_irq_no: usize, arg: *mut usize) {
    ...
    if let Some(conn) = xl.handler_conn {
        xous::try_send_message(
            ...
            xous::Message::new_scalar(
                Opcode::GpioIntHappened.to_usize().unwrap(),
                xl.gpio_csr.r(utra::gpio::EV_PENDING) as _,
                0,
                0,
                0,
            ),
    ...
    xl.gpio_csr.wo(utra::gpio::EV_PENDING, xl.gpio_csr.r(utra::gpio::EV_PENDING));
}
fn handle_power_irq(_irq_no: usize, arg: *mut usize) {
    ...
    if xl.power_csr.rf(utra::power::EV_PENDING_USB_ATTACH) != 0 {
        if let Some(conn) = xl.handler_conn {
            xous::try_send_message(
                conn,
                xous::Message::new_scalar(Opcode::EventUsbHappened.to_usize().unwrap(), 0, 0, 0, 0),
            )
            .map(|_| ())
            .unwrap();
        } else {
        ...
        if let Some(conn) = xl.handler_conn {
            ...
            xous::try_send_message(
                conn,
                xous::Message::new_scalar(
                    Opcode::EventActivityHappened.to_usize().unwrap(),
                    activity as usize,
                    0,
                    0,
                    0,
    ...
    xl.power_csr.wo(utra::power::EV_PENDING, xl.power_csr.r(utra::power::EV_PENDING));
}
...
impl Llio {
    ...
    pub(crate) fn init(&mut self) {
        ...
        xous::claim_interrupt(utra::gpio::GPIO_IRQ, handle_gpio_irq, self as *mut Llio as *mut usize)
            .expect("couldn't claim GPIO irq");

        xous::claim_interrupt(utra::power::POWER_IRQ, handle_power_irq, self as *mut Llio as *mut usize)
    ...
    pub(crate) fn get_power_csr_raw(&self) -> *mut u32 { self.power_csr_raw }
    ...
    pub fn activity_get_period(&mut self) -> u32 { self.activity_period }

    pub fn gpio_dout(&mut self, d: u32) { self.gpio_csr.wfo(utra::gpio::OUTPUT_OUTPUT, d); }

    pub fn gpio_din(&self) -> u32 { self.gpio_csr.rf(utra::gpio::INPUT_INPUT) }

    pub fn gpio_drive(&mut self, d: u32) { self.gpio_csr.wfo(utra::gpio::DRIVE_DRIVE, d); }

    pub fn gpio_int_mask(&mut self, d: u32) { self.gpio_csr.wfo(utra::gpio::INTENA_INTENA, d); }

    pub fn gpio_int_as_falling(&mut self, d: u32) { self.gpio_csr.wfo(utra::gpio::INTPOL_INTPOL, d); }

    pub fn gpio_int_pending(&self) -> u32 { self.gpio_csr.r(utra::gpio::EV_PENDING) & 0xff }

    pub fn gpio_int_ena(&mut self, d: u32) { self.gpio_csr.wo(utra::gpio::EV_ENABLE, d & 0xff); }
    ...
    pub fn set_uart_mux(&mut self, mux: UartType) {
        match mux {
            ...
            UartType::Log => {
                // this is a command mainly for debugging, so we'll accept the chance that we re-enabled WFI
                // e.g. during a critical operation like SPINOR flashing because we swapped
                // consoles at a bad time. Should be very rare and only affect devs...
                log::warn!(
                    "unsafe re-enabling WFI -- if you issued this command at a bad time, could have side effects"
                );
                if true {
    ...
    pub fn get_info_git(&self) -> (usize, usize) {
        (
            ((self.info_csr.rf(utra::info::GIT_MAJOR_GIT_MAJOR) as u32) << 24
                ...
                | (self.info_csr.rf(utra::info::GIT_GITEXTRA_GIT_GITEXTRA) as u32) & 0xFF << 0)
                as usize,
    ...
    pub fn get_info_platform(&self) -> (usize, usize) {
        (
            self.info_csr.r(utra::info::PLATFORM_PLATFORM0) as usize,
            self.info_csr.r(utra::info::PLATFORM_PLATFORM1) as usize,
        )
    }

    pub fn get_info_target(&self) -> (usize, usize) {
        (
            self.info_csr.r(utra::info::PLATFORM_TARGET0) as usize,
            self.info_csr.r(utra::info::PLATFORM_TARGET1) as usize,
        )
    }
    ...
    pub fn ec_power_on(&mut self) { self.power_csr.rmwf(utra::power::POWER_UP5K_ON, 1); }
    ...
    #[allow(dead_code)]
    pub fn tts_sleep_indicate(&mut self) { self.power_csr.wfo(utra::power::VIBE_VIBE, 1); }

    pub fn xadc_vbus(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_VBUS_XADC_VBUS) as u16 }

    pub fn xadc_vccint(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_VCCINT_XADC_VCCINT) as u16 }

    pub fn xadc_vccaux(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_VCCAUX_XADC_VCCAUX) as u16 }

    pub fn xadc_vccbram(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_VCCBRAM_XADC_VCCBRAM) as u16 }

    pub fn xadc_usbn(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_USB_N_XADC_USB_N) as u16 }

    pub fn xadc_usbp(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_USB_P_XADC_USB_P) as u16 }

    pub fn xadc_temperature(&self) -> u16 {
        self.xadc_csr.rf(utra::trng::XADC_TEMPERATURE_XADC_TEMPERATURE) as u16
    }

    pub fn xadc_gpio5(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_GPIO5_XADC_GPIO5) as u16 }

    pub fn xadc_gpio2(&self) -> u16 { self.xadc_csr.rf(utra::trng::XADC_GPIO2_XADC_GPIO2) as u16 }
