...
impl Keyboard {
    ...
    pub fn register_listener(&self, server_name: &str, action_opcode: usize) {
        ...
        buf.lend(self.conn, Opcode::RegisterListener.to_u32().unwrap()).expect("couldn't register listener");
    }
    ...
    pub fn set_keymap(&self, map: KeyMap) -> Result<(), xous::Error> {
        send_message(
            self.conn,
            Message::new_scalar(Opcode::SelectKeyMap.to_usize().unwrap(), map.into(), 0, 0, 0),
        )
        .map(|_| ())
    }
    ...
    pub fn get_keys_blocking(&self) -> Vec<char> {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::BlockingKeyListener.to_usize().unwrap(), 0, 0, 0, 0),
        ) {
    ...
    pub fn hostmode_inject_key(&self, c: char) {
        send_message(
            self.conn,
            Message::new_scalar(Opcode::InjectKey.to_usize().unwrap(), c as u32 as usize, 0, 0, 0),
        )
        .unwrap();
    }
