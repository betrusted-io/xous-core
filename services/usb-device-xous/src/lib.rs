...
impl UsbHid {
    ...
    pub fn is_soc_compatible(&self) -> bool {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::IsSocCompatible.to_usize().unwrap(), 0, 0, 0, 0),
        ) {
    ...
    pub fn switch_to_core(&self, core: UsbDeviceType) -> Result<(), xous::Error> {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::SwitchCores.to_usize().unwrap(), core as usize, 0, 0, 0),
        ) {
            Ok(xous::Result::Scalar1(code)) => match code {
                0 => Ok(()),
    ...
    pub fn ensure_core(&self, core: UsbDeviceType) -> Result<(), xous::Error> {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::EnsureCore.to_usize().unwrap(), core as usize, 0, 0, 0),
        ) {
            Ok(xous::Result::Scalar1(code)) => match code {
                0 => Ok(()),
                _ => Err(xous::Error::InternalError),
    ...
    pub fn get_current_core(&self) -> Result<UsbDeviceType, xous::Error> {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::WhichCore.to_usize().unwrap(), 0, 0, 0, 0),
        ) {
            Ok(xous::Result::Scalar1(code)) => match code {
    ...
    pub fn is_debug_restricted(&self) -> Result<bool, xous::Error> {
        match send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::IsRestricted.to_usize().unwrap(), 0, 0, 0, 0),
        ) {
    ...
    pub fn debug_usb(&self, do_lock: Option<bool>) -> Result<(bool, bool), xous::Error> {
        ...
        let (arg1, arg2) = if let Some(lock) = do_lock { if lock { (1, 1) } else { (1, 0) } } else { (0, 0) };
    ...
    pub fn serial_wait_ascii(&self, delimiter: Option<char>) -> String {
        let req = UsbSerialAscii { s: xous_ipc::String::new(), delimiter };
