...
impl AppsBlockDevice {
    pub fn new() -> AppsBlockDevice {
        ...
        let rw_ids = Arc::new(Mutex::new(RwOp { read_id: 0, write_id: 0, max_lba_id: 0 }));
                ...
                match opcode {
                    Some(BlockDeviceMgmtOp::SetOps) => {
                        msg_blocking_scalar_unpack!(msg, read_id, write_id, max_lba_id, _, {
                            ...
                            *ids = RwOp { read_id, write_id, max_lba_id };
                    ...
                    Some(BlockDeviceMgmtOp::Reset) => msg_blocking_scalar_unpack!(msg, 0, 0, 0, 0, {
                        ...
                        *ac = None;
                        *ids = RwOp { read_id: 0, write_id: 0, max_lba_id: 0 };
        ...
        AppsBlockDevice { app_cid, rw_ids, mgmt_cid: xous::connect(mgmt_sid).unwrap() }
    }

    pub fn conn(&self) -> xous::CID { self.mgmt_cid }
}
...
impl BlockDevice for AppsBlockDevice {
    ...
    fn read_block(&self, lba: u32, block: &mut [u8]) -> Result<(), usbd_scsi::BlockDeviceError> {
        ...
        let buf = unsafe {
            xous::MemoryRange::new(&mut request as *mut RawData as usize, core::mem::size_of::<RawData>())
                .unwrap()
        };

        xous::send_message(
            cid,
            xous::Message::new_lend_mut(rw_ids.read_id, buf, xous::MemoryAddress::new(lba as usize), None),
        )
        .unwrap();
    ...
    fn write_block(&mut self, lba: u32, block: &[u8]) -> Result<(), usbd_scsi::BlockDeviceError> {
        ...
        xous::send_message(
            cid,
            xous::Message::new_lend_mut(rw_ids.write_id, buf, xous::MemoryAddress::new(lba as usize), None),
        )
        .unwrap();
    ...
    fn max_lba(&self) -> u32 {
        ...
        match xous::send_message(cid, xous::Message::new_blocking_scalar(rw_ids.max_lba_id, 0, 0, 0, 0))
