...
fn xmain() -> ! {
    ...
    xous::create_thread_4(status_thread, gid[0] as _, gid[1] as _, gid[2] as _, gid[3] as _).expect("couldn't create status thread");

    let mut powerdown_requested = false;
    let mut last_time: u64 = ticktimer.elapsed_ms();
    log::trace!("entering main loop");
    loop {
        let msg = xous::receive_message(gam_sid).unwrap();
        ...
        match FromPrimitive::from_usize(msg.body.id()) {
                ...
                msg_scalar_unpack!(msg, g0, g1, g2, g3, {
                    ...
                    match canvases.get(&gid) {
                        Some(c) => {
                            ...
                            rect.style = DrawStyle {fill_color: Some(PixelColor::Light), stroke_color: None, stroke_width: 0,};
                            gfx.draw_rectangle(gfx_conn, rect).expect("can't clear canvas");
                        },
            ...
            None => panic!("unhandled message {:?}", msg)
        }
    }
}
