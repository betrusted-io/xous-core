...
use xous::{SID, msg_blocking_scalar_unpack, msg_scalar_unpack, Message};
use xous_ipc::Buffer;
use crate::NetConn;
use crate::api::*;
//use crate::api::udp::*;
...
impl DnsServerManager {
    pub fn register(xns: &xous_names::XousNames) -> Result<DnsServerManager> {
            ...
            move || {
                loop {
                    ...
                    match FromPrimitive::from_usize(msg.body.id()) {
                        Some(PrivateDnsOp::AddIpv4DnsServer) => msg_scalar_unpack!(msg, be_octets, _, _, _, {
                            let octets = (be_octets as u32).to_be_bytes();
                            let dns_ip = IpAddr::V4(Ipv4Addr::new(octets[0], octets[1], octets[2], octets[3]));
                        ...
                        Some(PrivateDnsOp::AddIpv6DnsServer) => msg_scalar_unpack!(msg, w0, w1, w2, w3, {
                            ...
                            let addr_bytes = [
                                w0_be[0], w0_be[1], w0_be[2], w0_be[3],
                                w1_be[0], w1_be[1], w1_be[2], w1_be[3],
                                w2_be[0], w2_be[1], w2_be[2], w2_be[3],
                                w3_be[0], w3_be[1], w3_be[2], w3_be[3],
                            ];
                            let dns_ip = IpAddr::V6(Ipv6Addr::from(addr_bytes));
                            servers.lock().unwrap().insert(dns_ip);
                        }),
                        Some(PrivateDnsOp::RemoveIpv4DnsServer) => msg_scalar_unpack!(msg, be_octets, _, _, _, {
                            let octets = (be_octets as u32).to_be_bytes();
                            let dns_ip = IpAddr::V4(Ipv4Addr::new(octets[0], octets[1], octets[2], octets[3]));
                            if !servers.lock().unwrap().remove(&dns_ip) {
                                log::warn!("Attempt to remove DNS server {:?} that isn't in our list. Ignored.", dns_ip);
                            }
                        }),
        ...
        let mut buf = Buffer::into_buf(hook)
    ...
    pub fn add_server(&mut self, addr: IpAddr) -> bool {
        if !self.freeze {
            self.servers.lock().unwrap().insert(addr)
        } else {
            false
        }
    }
    /// Returns true if the server was removed, false if the server wasn't in the table and thus couldn't be removed.
    ...
    pub fn get_random(&self) -> Option<IpAddr> {
        if let Some(&addr) = self.servers.lock().unwrap().iter().next() {
            Some(addr)
        } else {
            None
        }
