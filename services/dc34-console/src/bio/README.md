...
With a standalone Zig installation:

```text
zig build -Dmodule=<program>
```

With the `ziglang` Python package:

```text
# Linux and macOS
python3 -m ziglang build "-Dmodule=<program>"

# Windows
python -m ziglang build "-Dmodule=<program>"
```

To also assemble the errata-patched output into
`<program>/<program>.bin`, suitable for a host-side uploader, add
`-Demit-binary=true`:

```text
# Standalone Zig
zig build -Dmodule=<program> -Demit-binary=true

# Linux and macOS
python3 -m ziglang build "-Dmodule=<program>" "-Demit-binary=true"

# Windows
python -m ziglang build "-Dmodule=<program>" "-Demit-binary=true"
```

The build uses `python3` to run `clang2rustasm.py` on Linux and macOS and
`python` on Windows. Override the detected default for virtual environments or
custom installations with `-Dpython=<executable>`.

### Uploading a binary

The console's `bio` REPL command does not read a host filename directly. A
host-side uploader must encode the `.bin` as follows:

1. Reject binaries larger than the `0xf00`-byte BIO program space.
2. Split the binary into 64-byte chunks, zero-padding the final data chunk.
3. Prefix each chunk with its zero-based index as a big-endian `u16`.
4. Append the IEEE CRC32 of the index and data bytes as a big-endian `u32`.
5. Base64-encode the resulting 70-byte record and send it as the argument to
   the `bio` REPL command.

There are 60 chunk slots, indexed 0 through 59. Sending all 60 commits the
program automatically. For a shorter program, send only its data chunks and
then run `bio pad`; the console zero-fills the remaining slots and commits the
program. This repository does not currently include the host-side uploader.

## Details

#### Installing Zig

To install Zig through Python, use the launcher appropriate for your platform:
...
- `cd src/bio` from the repository root
- Run one of the platform-appropriate commands from the Quick start section
...
You can also run the clang-to-Rust converter stand-alone after the Zig assembly
step:

```text
# Linux and macOS
python3 clang2rustasm.py <program>

# Windows
python clang2rustasm.py <program>
```

#### Conventions

- A collection of useful header files are kept in `include`
- Each BIO C program is contained in its own directory inside `src/bio`
- `main.c` contains the C code
- `<program>.rs` is the Rust macro assembly - it is autogenerated
- A program exposed to Rust also provides its supporting Rust module files
- Add programs that should be callable from Rust to `src/bio/mod.rs`
