...
///
/// N.B.: As of Xous 0.9.6 we don't have a call to update the anti-rollback count, we have only provisioned
/// for that call to exist sometime in the future.
const MAX_ROLLBACK_LIMIT: u8 = 255;

/// Size of the total area allocated for signatures. It is equal to the size of one FLASH sector, which is the
/// smallest increment that can be erased.
...
pub(crate) fn bitflip(input: &[u8], output: &mut [u8]) {
    ...
    for (src, dst) in input.chunks(4).into_iter().zip(output.chunks_mut(4).into_iter()) {
...
pub(crate) struct RootKeys {
    ...
    /// Window for end of kernel. Needed for updating the appended kernel length field in the signing
    /// process.
    kernel_end_mr: xous::MemoryRange,
    kernel_end_base_offset: u32,
    kernel_base: u32,
    /// regions of RAM that holds all plaintext passwords, keys, and temp data. stuck in two well-defined
    /// page so we can zero-ize it upon demand, without guessing about stack frames and/or Rust
    /// optimizers removing writes
    sensitive_data: RefCell<xous::MemoryRange>, /* this gets purged at least on every suspend, but ideally
                                                 * purged sooner than that */
    ...
    cur_password_type: Option<PasswordType>, /* for tracking which password we're dealing with at the UX
                                              * layer */
    ...
    jtag: jtag::Jtag,
    fake_key: [u8; 32], /* a base set of random numbers used to respond to invalid keyloc requests in AES
                         * operations */
...
impl<'a> RootKeys {
    pub fn new() -> RootKeys {
        ...
        // match HEADER_TOTAL_SIZE, but part of the reason we hard code the number is because
        // KERNEL_BACKUP_OFFSET is in the xous crate (maybe it shouldn't be?) and is hard-coded to
        // 4096 (0x1000), so for now we have this assert to catch a consistency problem in assumptions
        // down the road.
        ...
        let mut sensitive_data =
            xous::syscall::map_memory(None, None, 0x1000, xous::MemoryFlags::R | xous::MemoryFlags::W)
        ...
        spinor
            .register_soc_token()
            .expect("couldn't register rootkeys as the one authorized writer to the gateware update area!");
    ...
    pub fn gateware(&self) -> &[u8] { unsafe { self.gateware_mr.as_slice::<u8>() } }

    pub fn gateware_base(&self) -> u32 { self.gateware_base }

    pub fn staging(&self) -> &[u8] { unsafe { self.staging_mr.as_slice::<u8>() } }

    pub fn staging_base(&self) -> u32 { self.staging_base }

    pub fn loader_code(&self) -> &[u8] { unsafe { self.loader_code_mr.as_slice::<u8>() } }

    pub fn loader_base(&self) -> u32 { self.loader_code_base }

    pub fn kernel_sig(&self) -> &[u8] { unsafe { self.kernel_sig_mr.as_slice::<u8>() } }

    pub fn kernel_backup(&self) -> &[u8] { unsafe { self.kernel_backup_mr.as_slice::<u8>() } }

    pub fn kernel_end(&self) -> &[u8] { unsafe { self.kernel_end_mr.as_slice::<u8>() } }

    pub fn kernel_hash(&self) -> Sha512Prehash {
        let mut ph = Sha512Prehash::new();
        if let Some(hash_str) =
            std::env::vars().find(|(key, _value)| key == "ROOT_FILESYSTEM_HASH").map(|(_key, value)| value)
        {
            log::info!("loader-generated hash environment string: {}", hash_str);
            let hash =
                hex::decode(hash_str).expect("Couldn't decode loader hash from environment variables!");
            for &b in hash.iter() {
                if b != 0 {
                    ...
                    Err(e) => {
                        kwp.result = Some(match e {
                            aes_kw::Error::IntegrityCheckFailed => {
                                // try the legacy version, if it unwraps, send back the key + an error that
                                // indicates the caller needs to update their version
                                    ...
                                    .decapsulate(&kwp.data[..kwp.len as usize], kwp.expected_len as usize)
                                {
                                    Ok(unwrapped) => {
                                        ...
                                        // hand the caller back a new version of the wrapped key that meets
                                        // NIST specs.
                                        let corrected = keywrapper
                                            .wrap_with_padding_vec(&unwrapped)
                                            .expect("couldn't convert to correct version of AES keywrapping");
                                        let mut upgrade = [0u8; 40];
                                        assert!(
                                            corrected.len() == 40,
                                            "Correctly wrapped key has a different length than the legacy wrapped key"
                                        );
                                        ...
                                        assert!(
                                            kwp.len == 32,
                                            "Unwrapped key from legacy version of algorithm has the wrong length"
                                        );
                                        unwrapped_key.copy_from_slice(&unwrapped);
                                        log::warn!(
                                            "Keywrap from incorrect version of algorithm; sending message to correct the problem"
                                        );
    ...
    pub fn is_zero_key(&self) -> Option<bool> {
        if let Some(secured) = self.is_efuse_secured() {
            if !secured {
                    ...
                    match self.jtag.efuse_fetch() {
                        Ok(record) => {
                            if record.key == [0u8; 32] {
                                ...
                                log::warn!(
                                    "Efuse key was set, and we're booting from it, but the readback protection was NOT enabled. The key is not secured."
                                );
                ...
                // we can't prove it's not zero, but for purposes of updates and provisioning, we should treat
                // it as non-zero.
                Some(false)
            }
        } else {
    ...
    pub fn is_jtag_working(&self) -> bool {
        if self.jtag.get_id().unwrap() == jtag::XCS750_IDCODE { true } else { false }
    }
    ...
    fn populate_sensitive_data(&mut self) {
        for (addr, d) in
            unsafe { self.sensitive_data.borrow_mut().as_slice_mut::<u32>().iter_mut().enumerate() }
        {
            if addr > 255 {
                break;
            }
    ...
    fn replace_fpga_key(&mut self) {
        ...
        if self.is_initialized() {
            assert!(
                pcache.hashed_update_pw_valid != 0,
                "update password was not set before calling replace_fpga_key"
            );
        }
        // build a new key into the pcache
        for dst_lw in pcache.fpga_key.chunks_mut(8).into_iter() {
            // 64 bit chunks
        ...
        // now encrypt the key, and store it into the sensitive_data for access later on by the patching
        // routine
        if self.is_initialized() {
            for (dst, (key, pw)) in unsafe {
                self.sensitive_data.borrow_mut().as_slice_mut::<u32>()
                    [KeyRomLocs::FPGA_KEY as usize..KeyRomLocs::FPGA_KEY as usize + 8]
                    .iter_mut()
            }
            .zip(pcache.fpga_key.chunks(4).into_iter().zip(pcache.hashed_update_pw.chunks(4).into_iter()))
            {
                *dst = u32::from_be_bytes(key[0..4].try_into().unwrap())
                    ^ u32::from_be_bytes(pw[0..4].try_into().unwrap());
            }
        } else {
            for (dst, key) in unsafe {
                self.sensitive_data.borrow_mut().as_slice_mut::<u32>()
                    [KeyRomLocs::FPGA_KEY as usize..KeyRomLocs::FPGA_KEY as usize + 8]
                    .iter_mut()
            }
            .zip(pcache.fpga_key.chunks(4).into_iter())
            {
                *dst = u32::from_be_bytes(key[0..4].try_into().unwrap());
            }
        }
    ...
    /// Plaintext password is passed as a &str. Any copies internally are destroyed. Caller is responsible for
    /// destroying the &str original. Performs a bcrypt hash of the password, with the currently set salt;
    /// does not store the plaintext after exit.
    pub fn hash_and_save_password(&mut self, pw: &str, verify: bool) -> bool {
        ...
        // we change the salt ever-so-slightly for every password. This doesn't make any one password more
        // secure; but it disallows guessing all the passwords with a single off-the-shelf hashcat
        // run.
    ...
    pub fn is_initialized(&mut self) -> bool {
        ...
        if config & keyrom_config::INITIALIZED.ms(1) != 0 { true } else { false }
    }

    pub fn is_pcache_update_password_valid(&self) -> bool {
        let pcache: &mut PasswordCache =
            unsafe { &mut *(self.pass_cache.as_mut_ptr() as *mut PasswordCache) };
        if pcache.hashed_update_pw_valid == 0 { false } else { true }
    }

    pub fn is_pcache_boot_password_valid(&self) -> bool {
        let pcache: &mut PasswordCache =
            unsafe { &mut *(self.pass_cache.as_mut_ptr() as *mut PasswordCache) };
        if pcache.hashed_boot_pw_valid == 0 { false } else { true }
    }
    ...
    pub fn setup_key_init(&mut self) {
        ...
        for keyword in unsafe { self.sensitive_data.borrow_mut().as_slice_mut::<u32>() }
            [KeyRomLocs::PEPPER as usize..KeyRomLocs::PEPPER as usize + 128 / (size_of::<u32>() * 8)]
            .iter_mut()
        {
    ...
    pub fn setup_restore_init(&mut self, key: backups::BackupKey, rom: backups::KeyRomExport) {
        ...
        // populate the staging area, in particular we are interested in the "pepper" so passwords work
        // correctly.
        unsafe { self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[..256].copy_from_slice(&rom.0) };
        ...
        unsafe {
            self.sensitive_data.borrow_mut().as_slice_mut::<u8>()
                [KeyRomLocs::FPGA_KEY as usize..KeyRomLocs::FPGA_KEY as usize + 32]
    ...
    /// is embedded all over the place as a substitute for `sensitive_slice` because we need interior
    /// mutability of the data within the MemoryRange (which is not mutable) to change the contents of the
    /// MemoryRange (a mutable operation). We can't bind `sensitive_slice` to
    /// `self.sensitive_data.borrow_mut().as_slice_mut::<u32>()` because this creates a temporary that has
    /// the wrong lifetime, and thus, we have to embed that terrible piece of unmaintainable syntax all
    /// over the place in the code below to solve this problem.
    ///
    /// If `restore` is `Some`, don't generate keys, but restore from backup. The key provided to this routine
    /// is *always* the correct key for the FPGA to boot from. The entry in the KeyRom will be adjusted
    /// accordingly.
    pub fn do_key_init(
        &mut self,
        ...
        let mut progress_action =
            Slider::new(main_cid, Opcode::UxGutter.to_u32().unwrap(), 0, 100, 10, Some("%"), 0, true, true);
        progress_action.set_is_password(true);
        // now show the init wait note...
        rootkeys_modal.modify(
            Some(ActionType::Slider(progress_action)),
            Some(t!("rootkeys.setup_wait", locales::LANG)),
            false,
            None,
            true,
            None,
        ...
        let keypair = if true {
            // true for production, false for debug (uses dev keys, so we can compare results)
            ...
            // hash a derived secret key based on the maximum rollback limit we anticipate less the current
            // global rollback limit (which is 0) this allows us to work with a "final" key that
            // generates a private signing key, which can't be predicted to future versions
            // given the current key (due to the irreversibility of Sha512/256), but once derived to a more
            // recent version can be willfully computed to an older version to recover e.g. old
            // data encrypted with a prior key
            //
            // We don't use the `compute_key_rollback` function because the value of the GLOBAL_ROLLBACK in
        ...
        // 0's key and we'd later on set it)
        #[cfg(feature = "hazardous-debug")]
        self.debug_print_key(KeyRomLocs::FPGA_KEY as usize, 256, "FPGA key before encryption: ");
        // before we encrypt it, stash a copy in our password cache, as we'll need it later on to encrypt the
        // bitstream
        ...
        for (word, hashed_pass) in unsafe {
            self.sensitive_data.borrow_mut().as_slice_mut::<u32>()
                [KeyRomLocs::FPGA_KEY as usize..KeyRomLocs::FPGA_KEY as usize + 256 / (size_of::<u32>() * 8)]
                .iter_mut()
        }
        ...
        let mut dst_oracle = match BitstreamOracle::new(
            &pcache.fpga_key,
            &pcache.fpga_key,
            self.gateware(),
            self.gateware_base(),
        ...
        let mut private_key_enc: [u8; ed25519_dalek::SECRET_KEY_LENGTH] =
        ...
        for (src, dst) in private_key_enc.chunks(4).into_iter().zip(unsafe {
            self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[KeyRomLocs::SELFSIGN_PRIVKEY as usize
                ..KeyRomLocs::SELFSIGN_PRIVKEY as usize + 256 / (size_of::<u32>() * 8)]
                .iter_mut()
        }) {
            *dst = u32::from_be_bytes(src.try_into().unwrap())
        }
        ...
        for dst in boot_key_enc.chunks_mut(4).into_iter() {
        ...
        for (src, dst) in boot_key_enc.chunks(4).into_iter().zip(unsafe {
            self.sensitive_data.borrow_mut().as_slice_mut::<u32>()
                [KeyRomLocs::USER_KEY as usize..KeyRomLocs::USER_KEY as usize + 256 / (size_of::<u32>() * 8)]
                .iter_mut()
        }) {
            *dst = u32::from_be_bytes(src.try_into().unwrap())
        }
        ...
        unsafe {
            self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[KeyRomLocs::CONFIG as usize] |=
                keyrom_config::INITIALIZED.ms(1)
        };
        ...
        self.spinor
            .patch(
                self.gateware(),
                self.gateware_base(),
                &self.staging()[METADATA_OFFSET..SELFSIG_OFFSET],
        ...
        for w in private_key_enc.iter_mut() {
            // it's encrypted. but i still want it turned to zeroes.
            *w = 0;
        }
        self.purge_password(PasswordType::Boot);
        self.purge_password(PasswordType::Update);
    ...
    pub fn printkeys(&mut self) {
        // dump the keystore -- used to confirm that patching worked right. does not get compiled in when
        // hazardous-debug is not enable.
        for addr in 0..256 {
            self.keyrom.wfo(utra::keyrom::ADDRESS_ADDRESS, addr);
            self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[addr as usize] =
                self.keyrom.rf(utra::keyrom::DATA_DATA);
    ...
    pub fn do_gateware_update(
        ...
        let mut pb = if update_type == UpdateType::Restore {
            ...
            rootkeys_modal.modify(
                Some(ActionType::Slider(progress_action)),
                Some(t!("rootkeys.gwup_starting", locales::LANG)),
                false,
                None,
                true,
                None,
            ...
            for (key, (&enc_key, &pw)) in keypair_bytes[..ed25519_dalek::SECRET_KEY_LENGTH]
                .iter_mut()
            ...
            #[cfg(feature = "hazardous-debug")]
            log::debug!(
                "keypair privkey (after anti-rollback): {:x?}",
                &keypair_bytes[..ed25519_dalek::SECRET_KEY_LENGTH]
            );
            // read in the public key from the staged data
            for (key, &src) in keypair_bytes[ed25519_dalek::SECRET_KEY_LENGTH..]
                .iter_mut()
            ...
            // if we can create the oracle, it means we were able to decrypt the first little bit of the boot
            // image and we're good to go!
            ...
            for (word, hashed_pass) in unsafe {
                self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[KeyRomLocs::FPGA_KEY as usize
                    ..KeyRomLocs::FPGA_KEY as usize + 256 / (size_of::<u32>() * 8)]
                    .iter_mut()
            }
            .zip(pcache.hashed_update_pw.chunks(4).into_iter())
            {
            ...
            // derived_sk now holds the "Root" secret key. It needs to be hashed (MAX_ROLLBACK_LIMIT -
            // GLOBAL_ROLLBACK) times to get the current signing key.
            self.compute_key_rollback(&mut keypair_bytes[..ed25519_dalek::SECRET_KEY_LENGTH]);
            // now populate the public key portion. that's just in the plain.
            // note that this would have been updated in the case of an update to GLOBAL_ROLLBACK -- the
            // purpose of this routine is to sign software in the current rollback count, not to
            ...
            if update_type == UpdateType::BbramProvision || update_type == UpdateType::EfuseProvision {
                if self.is_initialized() {
                    // make a backup copy of the old key, so we can use it to decrypt the gateware before
                    // re-encrypting it
                    old_key.copy_from_slice(&pcache.fpga_key);
                }
                ...
                if update_type == UpdateType::BbramProvision {
                    ...
                    modals
                        .show_bip39(Some(t!("rootkeys.backup_key", locales::LANG)), &pcache.fpga_key.to_vec())
                        .ok();
                    loop {
        ...
        if keypair.is_some() {
            log::debug!(
                "keypair privkey (after anti-rollback): {:x?}",
                keypair.as_ref().unwrap().secret.to_bytes()
            );
        }
        ...
        log::debug!("making destination oracle");
        let mut dst_oracle =
            match BitstreamOracle::new(&old_key, &pcache.fpga_key, self.gateware(), self.gateware_base()) {
                Ok(o) => o,
                Err(e) => {
                    self.purge_password(PasswordType::Update);
                    log::error!("couldn't create oracle (most likely FPGA key mismatch): {:?}", e);
        ...
        log::debug!("making source oracle");
        let mut src_oracle =
            match BitstreamOracle::new(&dummy_key, &pcache.fpga_key, self.staging(), self.staging_base()) {
                Ok(o) => o,
                Err(e) => {
                    log::error!("couldn't create oracle (most likely FPGA key mismatch): {:?}", e);
                    dst_oracle.clear();
        ...
        let keypair = if let Some(kp) = keypair {
            ...
            // pass the kp back into the original variable. keypair does not implement copy...for good
            // reasons.
            Some(kp)
        } else {
            None
        ...
        if update_type == UpdateType::BbramProvision {
            ...
            // the key burning routine should finish before this timeout happens, and the system should have
            // been rebooted
            ...
            match self.jtag.efuse_key_burn(pcache.fpga_key) {
                ...
                Err(e) => {
                    pb.update_text(&format!(
                        "{}\n{:?}",
                        t!("rootkeys.efuse_internal_error", locales::LANG),
                        e
                    ));
                    self.ticktimer.sleep_ms(2000).ok();
    ...
    pub fn test(&mut self, rootkeys_modal: &mut Modal, main_cid: xous::CID) -> Result<(), RootkeyResult> {
        ...
        // the stop emoji, when sent to the slider action bar in progress mode, will cause it to close and
        // relinquish focus
        ...
        #[cfg(feature = "hazardous-debug")]
        {
            ...
            for (word, key) in self.sensitive_data.borrow().as_slice::<u32>()
                [KeyRomLocs::FPGA_KEY as usize..KeyRomLocs::FPGA_KEY as usize + 256 / (size_of::<u32>() * 8)]
                .iter()
            ...
            // copy from staging
            let src_oracle =
                BitstreamOracle::new(&pcache.fpga_key, &pcache.fpga_key, self.staging(), self.staging_base())
                    .unwrap();
            // write to gateware
            let dst_oracle = BitstreamOracle::new(
                &pcache.fpga_key,
                &pcache.fpga_key,
                self.gateware(),
                self.gateware_base(),
    ...
    pub(crate) fn do_gateware_provision_uninitialized(
        &mut self,
        rootkeys_modal: &mut Modal,
        main_cid: xous::CID,
    ) -> Result<(), RootkeyResult> {
    ...
    fn gateware_copy_and_patch(
        &self,
        src_oracle: &BitstreamOracle,
        dst_oracle: &BitstreamOracle,
        mut maybe_pb: Option<&mut ProgressBar>,
    ) -> Result<(), RootkeyResult> {
        log::debug!(
            "sanity checks: src_offset {}, dst_offset {}, src_len {}, dst_len {}",
            src_oracle.ciphertext_offset(),
            dst_oracle.ciphertext_offset(),
            src_oracle.ciphertext_len(),
            dst_oracle.ciphertext_len()
        );
        ...
        for (dst, (&hm1, &mask)) in
            hmac_code.iter_mut().zip(pt_sector[0..32].iter().zip(pt_sector[32..64].iter()))
        {
            *dst = hm1 ^ mask;
        }
        ...
        // hash the first sector; the pt_sector could have been patched by encrypt_sector() to change the key
        // soucre, so it must be done *after* we call encrypt_sector()
        ...
        self.spinor
            .patch(dst_oracle.bitstream(), dst_oracle.base(), &ct_sector, 0)
            .map_err(|_| RootkeyResult::FlashError)?;
        ...
        if let Some(ref mut pb) = maybe_pb {
            pb.rebase_subtask_work(
                0,
                src_oracle.ciphertext()[from as usize..]
                    .chunks(spinor::SPINOR_ERASE_SIZE as usize)
                    .into_iter()
                    .count() as u32,
            );
        }
        for _ in
            src_oracle.ciphertext()[from as usize..].chunks(spinor::SPINOR_ERASE_SIZE as usize).into_iter()
        {
            ...
            if self.patch_in_range(src_oracle, from, from + spinor::SPINOR_ERASE_SIZE as u32) {
                dummy_consume ^= self.patch_sector(src_oracle, from, &mut pt_sector, unsafe {
                    &self.sensitive_data.borrow_mut().as_slice_mut::<u32>()[0..256]
                });
            }

            let hash_len =
                if bytes_hashed + decrypt_len < hash_stop { decrypt_len } else { hash_stop - bytes_hashed };
            ...
            self.spinor
                .patch(
                    dst_oracle.bitstream(),
                    dst_oracle.base(),
                    &ct_sector[..decrypt_len],
                    from + dst_oracle.ciphertext_offset() as u32,
                )
                .map_err(|_| RootkeyResult::FlashError)?;
        ...
        for (dst, (&hm2, &mask)) in masked_footer.iter_mut().zip(hmac_code.iter().zip(footer_mask.iter())) {
            *dst = hm2 ^ mask;
        }
        ...
        // the math below will have some problems if the block happens to end up exactly on an erase block
        // boundary. but it doesn't, so I'm going to leave that case unaddressed...
        let ct_last_block_loc =
            (ct_end & !(spinor::SPINOR_ERASE_SIZE as usize - 1)) - dst_oracle.ciphertext_offset();
        let pt_sector_len = ct_end - ct_last_block_loc;

        log::trace!(
            "ct_end: {}, ct_last_block_loc {}, pt_sector_len {}",
            ct_end,
            ct_last_block_loc,
            pt_sector_len
        );
        ...
        for (&src, dst) in
            h2_digest_flipped.iter().zip(pt_sector[pt_sector_len - 32..pt_sector_len].iter_mut())
        {
            *dst = src;
        }
        ...
        dst_oracle.encrypt_sector(
            ct_last_block_loc as i32,
            &mut pt_sector[..pt_sector_len],
            &mut ct_sector[..pt_sector_len],
        );
        ...
        self.spinor
            .patch(
                dst_oracle.bitstream(),
                dst_oracle.base(),
                &ct_sector[..pt_sector_len],
                ct_last_block_loc as u32 + dst_oracle.ciphertext_offset() as u32,
            )
            .map_err(|_| RootkeyResult::FlashError)?;
    ...
    fn patch_in_range(&self, oracle: &BitstreamOracle, range_ct_start: u32, range_ct_end: u32) -> bool {
        ...
        if end_frame < patch::PATCH_FRAMES[0] as usize
            || start_frame > *patch::PATCH_FRAMES.last().unwrap() as usize
        {
            false
        } else {
            true
            ...
            if patch::should_patch(frame as u32) {
                match patch::patch_frame(frame as u32, frame_offset as u32, keyrom) {
                    Some((patch, patch_inv)) => {
                        ...
                        log::trace!(
                            "patching {} frame 0x{:x} offset {} 0x{:x} -> 0x{:x}",
                            ct_offset,
                            frame,
                            frame_offset,
                            u32::from_be_bytes(word[0..4].try_into().unwrap()),
                            u32::from_be_bytes(flip)
                        );
    ...
    fn verify_gateware(
        ...
        let mut hmac_code: [u8; 32] = [0; 32];
        for (dst, (&hm1, &mask)) in
            hmac_code.iter_mut().zip(hmac_area[0..32].iter().zip(hmac_area[32..64].iter()))
        {
            *dst = hm1 ^ mask;
        }
        ...
        // for now, very tiny optimization - use blocksize * 2 because that happens to be divisible by the
    ...
    fn make_gateware_backup(
        ...
        for (dst, src) in gateware_dest.chunks(PATCH_CHUNK).into_iter().zip(gateware_src.chunks(PATCH_CHUNK))
        {
            ...
            self.spinor.patch(dst, gateware_dest_base, src, 0).map_err(|_| RootkeyResult::FlashError)?;
    ...
    ///   - this helps keep kernel updates faster and lighter, at the trade-off of some inconvenience of
    ///     loader updates
    ...
    /// ed25519-dalek's version of the signature, but for interoperability we have to stick with the
    /// traditional signature. Unfortunately, the traditional signature implementation only supports
    /// passing the entire region as a slice, and not as a pre-hash. This is because the hash has to be
    ...
    pub fn sign_kernel(&self, signing_key: &Keypair) -> (Signature, u32) {
        ...
        // First, find the advertised length in the unchecked header, then, check it against the length in the
        // signed region of the kernel
        ...
        let protected_len = u32::from_le_bytes(
            kernel_end_region[(SIGBLOCK_SIZE as usize + kernel_len as usize - 4) & end_mask
                ..(SIGBLOCK_SIZE as usize + kernel_len as usize) & end_mask]
                .try_into()
                .unwrap(),
        );

        // we now have a checked length derived from a region of the kernel that is signed, confirm that it
        // matches the advertised length
        log::info!(
            "kernel total signed len 0x{:x}, kernel len inside signed region 0x{:x}",
            kernel_len,
            protected_len
        );
        // throw a panic -- this check should have passed during boot.
        assert!(
            (kernel_len) - 4 == protected_len as usize,
            "The advertised kernel length does not match the signed length"
        );
        ...
        log::info!(
            ...
            &(self.kernel_end()[(SIGBLOCK_SIZE as usize + kernel_len - 8) & end_mask
    ...
    pub fn verify_gateware_self_signature(&mut self, maybe_pubkey: Option<&PublicKey>) -> bool {
        ...
        let mut sig_region: [u8; core::mem::size_of::<SignatureInFlash>()] =
            [0; core::mem::size_of::<SignatureInFlash>()];
        for (&src, dst) in gateware_region
            [SELFSIG_OFFSET..SELFSIG_OFFSET + core::mem::size_of::<SignatureInFlash>()]
            .iter()
        ...
        // note that we use the *prehash* version here, this has a different signature than a straightforward
        // ed25519
    ...
    /// This function does a comprehensive check of all the possible signature types in a specified gateware
    /// region
    pub fn check_gateware_signature(&mut self, region_enum: GatewareRegion) -> SignatureResult {
        ...
        loop {
            let pubkey_bytes = match sigtype {
                SignatureResult::SelfSignOk => self.read_key_256(KeyRomLocs::SELFSIGN_PUBKEY),
                SignatureResult::ThirdPartyOk => self.read_key_256(KeyRomLocs::THIRDPARTY_PUBKEY),
                SignatureResult::DevKeyOk => self.read_key_256(KeyRomLocs::DEVELOPER_PUBKEY),
                _ => return SignatureResult::InvalidSignatureType,
            };
    ...
    pub fn fetch_gw_metadata(&self, region_enum: GatewareRegion) -> MetadataInFlash {
        ...
        let md_ptr: *const MetadataInFlash = (&region
            [METADATA_OFFSET..METADATA_OFFSET + core::mem::size_of::<MetadataInFlash>()])
            .as_ptr() as *const MetadataInFlash;
    ...
    pub fn commit_signature(
        ...
        let signature: &mut SignatureInFlash =
            unsafe { (sig_region.as_mut_ptr() as *mut SignatureInFlash).as_mut().unwrap() }; // this pointer better not be null, we just created it!
        ...
        match sig_type {
            SignatureType::Loader => {
                ...
                self.spinor
                    .patch(self.loader_code(), self.loader_base(), &sig_region, 0)
                    .map_err(|_| RootkeyResult::FlashError)?;
            ...
            SignatureType::Kernel => {
                ...
                self.spinor
                    .patch(self.kernel_sig(), self.kernel_sig_base(), &sig_region, 0)
                    .map_err(|_| RootkeyResult::FlashError)?;
            ...
            SignatureType::Gateware => {
                signature.version = SIG_LOADER_VERSION;
                log::info!(
                    "gateware sig area before: {:x?}",
                    &(self.gateware()
                        [SELFSIG_OFFSET..SELFSIG_OFFSET + core::mem::size_of::<SignatureInFlash>()])
                );
                self.spinor
                    .patch(self.gateware(), self.gateware_base(), &sig_region, (SELFSIG_OFFSET) as u32)
                    .map_err(|_| RootkeyResult::FlashError)?;
                log::info!(
                    "gateware sig area after: {:x?}",
                    &(self.gateware()
                        [SELFSIG_OFFSET..SELFSIG_OFFSET + core::mem::size_of::<SignatureInFlash>()])
                );
            }
    ...
    pub fn finish_key_init(&mut self) {
        ...
        self.purge_sensitive_data();
        // reset this flag to false in case this was called at the end of a restore op; harmless if it's
        // already false.
    ...
    pub fn staged_semver(&self) -> SemVer {
        ...
        if staging_meta.magic == 0x6174656d {
            let tag_str =
                std::str::from_utf8(&staging_meta.tag_str[..(staging_meta.tag_len as usize).min(64)])
                    .unwrap_or("v0.0.0-1");
    ...
    pub fn try_nokey_soc_update(&mut self, rootkeys_modal: &mut Modal, main_cid: xous::CID) -> bool {
        ...
        let tag_str =
            std::str::from_utf8(&soc_meta.tag_str[..soc_meta.tag_len as usize]).unwrap_or("v0.0.0-1");
        let soc_sv =
        ...
        if staged_sv > soc_sv {
            ...
            log::warn!(
                "No-touch update attempted, but the staged version is not newer than the existing version. Aborting."
            );
            false
        }
    }
    ...
    pub fn should_prompt_for_update(&mut self) -> bool {
        ...
        if status == 0xFFFF_FFFF { true } else { false }
    }

    /// the update prompt is reset every time you stage a new update
    pub fn set_prompt_for_update(&mut self, state: bool) {
        let patch_data = if state { [0xffu8; 4] } else { [0x0u8; 4] };
        self.spinor
            .patch(self.staging(), self.staging_base(), &patch_data, self.staging().len() as u32 - 4)
    ...
    pub fn set_dont_ask_init(&mut self) {
        self.spinor
            .patch(self.gateware(), self.gateware_base(), &[0u8; 4], self.gateware().len() as u32 - 4)
            .expect("couldn't erase backup region");
    }

    pub fn reset_dont_ask_init(&mut self) {
        self.spinor
            .patch(self.gateware(), self.gateware_base(), &[0xffu8; 4], self.gateware().len() as u32 - 4)
            .expect("couldn't erase backup region");
    }
    ...
    pub fn get_backup_key(&mut self) -> Option<(backups::BackupKey, backups::KeyRomExport)> {
        ...
        match keypair.verify(&test_data.as_bytes(), &test_sig) {
            Ok(_) => {
                for (&src, dst) in
                    self.read_key_256(KeyRomLocs::FPGA_KEY).iter().zip(pcache.fpga_key.iter_mut())
                {
                    *dst = src;
                }
                log::debug!("fpga key (encrypted): {:x?}", &pcache.fpga_key);
                ...
                pcache.fpga_key_valid = 1;
                // copy the plaintext FPGA key into the backup key structure, which implements the zeroize
                // trait.
    ...
    pub fn write_backup(
        ...
        block[size_of::<BackupHeader>()..size_of::<BackupHeader>() + size_of::<backups::BackupDataCt>()]
            .copy_from_slice(backup_ct.as_ref());
        ...
        if let Some(cs) = checksums {
            ...
            assert!(
                block.len() + cs_slice.len() < PAGE_LEN,
                "Error: checksum block has outgrown the available space"
            );
            page[PAGE_LEN - cs_slice.len()..].copy_from_slice(cs_slice);
        }
        self.spinor
            .patch(self.kernel_backup(), self.kernel_backup_base(), &page, 0)
    ...
    pub fn write_restore_dna(
        ...
        self.spinor
            .patch(self.kernel_backup(), self.kernel_backup_base(), &block, 0)
            .map_err(|_| xous::Error::InternalError)?;
        Ok(())
    }
    ...
    pub fn read_backup(&mut self) -> Result<(BackupHeader, backups::BackupDataCt), xous::Error> {
        ...
        header.as_mut().copy_from_slice(&self.kernel_backup()[..size_of::<BackupHeader>()]);
        ct.as_mut().copy_from_slice(
            &self.kernel_backup()
                [size_of::<BackupHeader>()..size_of::<BackupHeader>() + size_of::<backups::BackupDataCt>()],
        );
        Ok((header, ct))
    }
    ...
    pub fn erase_backup(&mut self) {
        ...
        self.spinor
            .patch(self.kernel_backup(), self.kernel_backup_base(), &blank, 0)
            .expect("couldn't erase backup region");
    }
}
