...
0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
0x00, 0x00, 0x00, 0xbb, 0x11, 0x22, 0x00, 0x44, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
0xaa, 0x99, 0x55, 0x66, 0x20, 0x00, 0x00, 0x00, 0x30, 0x03, 0xe0, 0x01, 0x00, 0x00, 0x01, 0x3b, # << this row
0x30, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x12, 0x20, 0x00, 0x00, 0x00, 0x30, 0x00, 0xc0, 0x01,
0x80, 0x00, 0x00, 0x40, 0x30, 0x00, 0xa0, 0x01, 0x80, 0x00, 0x00, 0x40, 0x30, 0x01, 0xc0, 0x01,
0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
...
0xaa, 0x99, 0x55, 0x66, 0x20, 0x00, 0x00, 0x00, 0x30, 0x03, 0xe0, 0x01, 0x00, 0x00, 0x00, 0x0b,
0x30, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x12, 0x20, 0x00, 0x00, 0x00, 0x30, 0x00, 0xc0, 0x01,
0x00, 0x00, 0x00, 0x40, 0x30, 0x00, 0xa0, 0x01, 0x00, 0x00, 0x00, 0x40, 0x30, 0x01, 0xc0, 0x01, # << this row
0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x30, 0x01, 0x60, 0x04,
...
def patcher(bit_in, patch):
    ...
    if len(patch) > 0:
        ...
        logging.debug("start of type 2 data: %d, %d (abs)", position, position + 64)
        ...
        for line in patch:
            ...
            for index in range(len(d)):
                if d[index].strip() != 'none':
                    ...
                    temp_pos = position + frame_num * 101 * 4 + index * 4
                    print("patching {} frame {} offset {} 0x{} -> 0x{}".format(temp_pos, hex(frame_num), index,
                        hex(int.from_bytes(ostream[temp_pos:temp_pos+4], byteorder='big')), binascii.hexlify(data)))
                    for b in range(4):
...
def main():
    ...
    new_iv = int(0x7c2069a18e10c06ede7b033b5a7dc437).to_bytes(16, byteorder='big')
    #new_hmac = get_random_bytes(32)
    new_hmac = int(0xac3d351af151e7d658cd6f26695ddb8b6b0b4af4f08dbff89e9531a4e1d3934c).to_bytes(32, byteorder='big')
    ...
    with open(ofile + ".bin", "wb") as f:
        ...
        logging.debug("plaintext len: %d", len(plaintext))
        logging.debug("initial plaintext: %s", binascii.hexlify(plaintext[:256]))
        ...
        logging.debug("keyed_footer (not flipped): %s", binascii.hexlify(keyed_footer))
        logging.debug("footer (not flipped): %s", binascii.hexlify(footer))
        logging.debug("digest: %s", binascii.hexlify(h1_digest))
        h2 = SHA256.new()
