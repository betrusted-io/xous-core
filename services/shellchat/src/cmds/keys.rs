...
impl Keys {
    pub fn new(xns: &xous_names::XousNames) -> Keys {
        ...
        #[cfg(all(not(any(feature="precursor", feature="renode")), feature="spinortest"))] // just make a dummy mapping to keep things from crashing in hosted mode
        let testing_range = xous::syscall::map_memory(
            None,
            None,
            TEST_SIZE,
    ...
    fn process(&mut self, args: String::<1024>, _env: &mut CommonEnv) -> Result<Option<String::<1024>>, xous::Error> {
        ...
        if let Some(sub_cmd) = tokens.next() {
            match sub_cmd {
                ...
                "aes" => {
                    use root_keys::{BlockEncrypt, BlockDecrypt};
                    let mut pass = true;
