//! # Display backend for bao1x target
//!
...
//! passing the blit initiation command off to the `bao1x-hal-service` server,
...
use bao1x_hal::board::setup_display_pins;
use bao1x_hal::udma;
use bao1x_hal::udma::PeriphId;
use bao1x_hal_service::IoxHal;
...
impl XousDisplay {
    pub fn new(_main_thread_token: MainThreadToken) -> XousDisplay {
        let udma_global = bao1x_hal_service::UdmaGlobal::new();
    ...
    pub fn suspend(&mut self) {
        unimplemented!("bao1x platform does not yet support suspend/resume");
    }
