#![allow(dead_code)]
// MacOS is broken in that it does not allow drawing from anything
// but the main thread. So the trick used to reduce UI latency on other
// OSes implemented in `minifb` by putting the redraw method in its
// own new thread does not work. This module provides a single-threaded
// implementation for MacOS so that hosted mode is at least functional for the
// platform, although laggy.
