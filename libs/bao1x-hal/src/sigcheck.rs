...
pub fn validate_image(
    ...
    // It is safe to check the anti-rollback counter before the signature because the
    // anti-rollback field is part of the signed data. Thus even if the anti-rollback data
    // is forged, it should be caught due to a failing signature check later on.
    ...
    match arb_offset {
        Some(arb) => {
            ...
            if arb_value > sig.sealed_data.anti_rollback {
                return Err(String::from("Anti-rollback code too old, refusing image"));
            }
            bollard!(die_no_std, 4);
            if arb_value < sig.sealed_data.anti_rollback {
                csprng.as_deref_mut().map(|rng| rng.random_delay());
                ...
                while one_way_counters.get(arb).unwrap() < sig.sealed_data.anti_rollback {
                    bollard!(die_no_std, 4);
                    // safety: anti-rollback counter argument is from a set of constants in bao1x_api that
                    // are pre-validated.
