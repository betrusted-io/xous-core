...
// It is gone in bao1x and we use the UDMA system reset instead
#[cfg(feature = "mpw")]
...
impl<'a> I2cDriver<'a> {
    ...
    pub unsafe fn new_with_ifram(
        ...
        // reset the block, if MPW. If bao1x, this is handled by udma global
        #[cfg(feature = "mpw")]
        unsafe {
    ...
    pub fn i2c_await(&mut self, rx_buf: Option<&mut [u8]>, _use_yield: bool) -> Result<usize, xous::Error> {
        ...
        while self.busy() {
            ...
            if timeout > TIMEOUT_ITERS {
                ...
                // reset the block, if MPW. If bao1x, this needs to be handled by the upper level code with a
