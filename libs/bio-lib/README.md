...
The BIO now supports a C-to-BIO code flow. It relies on the Zig C toolchain,
installable via Python.

The intended flow is for developers to write their BIO modules in C, and translate
these into a Rust macro that is then incorporated into the `bio-lib` ecosystem.
Thus, we adopt the following policy assumptions:

- The resulting Rust macro is checked into `xous-core` as the buildable artifact by most users
- Only developers looking to modify or create C code need to install the Zig toolchain
- It's assumed that you have Python 3.8 or later and a system that can support Zig's prerequisites
