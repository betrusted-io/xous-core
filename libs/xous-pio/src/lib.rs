...
impl SmConfig {
    pub fn default() -> SmConfig {
        ...
        SmConfig { clkdiv: 0x1_0000, execctl: 31 << 12, shiftctl: (1 << 18) | (1 << 19), pinctl: 0 }
    }
}
...
impl LoadedProg {
    ...
    pub fn entry(&self) -> usize {
        if let Some(ep) = self.entry_point { ep + self.offset } else { self.start() }
    }

    pub fn start(&self) -> usize { self.program.wrap.target as usize + self.offset }

    pub fn end(&self) -> usize { self.program.wrap.source as usize + self.offset }
...
impl PioSharedState {
    ...
    #[allow(dead_code)]
    pub unsafe fn free_sm(&mut self, sm: PioSm) { self.used_machines[sm.sm_index()] = false; }
    ...
    pub fn add_program(&mut self, program: &Program<RP2040_MAX_PROGRAM_SIZE>) -> Result<usize, PioError> {
    ...
    #[allow(dead_code)]
    pub fn remove_program(&mut self, program: &Program<RP2040_MAX_PROGRAM_SIZE>, loaded_offset: usize) {
    ...
    #[allow(dead_code)]
    pub fn clear_instruction_memory(&mut self) {
        self.used_mask = 0;
...
impl PioSm {
    #[allow(dead_code)]
    pub fn dbg_get_shiftctl(&self) -> u32 { self.config.shiftctl }
    ...
    pub fn sm_bitmask(&self) -> u32 { self.sm as u32 }
    ...
    pub fn config_set_set_pins(&mut self, set_base: usize, set_count: usize) {
        assert!(set_base < 32);
        assert!(set_count <= 5);
        self.config.pinctl = self.pio.zf(
            rp_pio::SFR_SM0_PINCTRL_PINS_SET_COUNT,
            self.pio.zf(rp_pio::SFR_SM0_PINCTRL_PINS_SET_BASE, self.config.pinctl),
    ...
    pub fn config_set_in_pins(&mut self, in_base: usize) {
        assert!(in_base < 32);
        self.config.pinctl = self.pio.zf(rp_pio::SFR_SM0_PINCTRL_PINS_IN_BASE, self.config.pinctl)
    ...
    pub fn config_set_sideset_pins(&mut self, sideset_base: usize) {
        ...
        self.config.pinctl = self.pio.zf(rp_pio::SFR_SM0_PINCTRL_PINS_SIDE_BASE, self.config.pinctl)
    ...
    pub fn config_set_sideset(&mut self, bit_count: usize, optional: bool, pindirs: bool) {
        ...
        self.config.pinctl = self.pio.zf(rp_pio::SFR_SM0_PINCTRL_PINS_SIDE_COUNT, self.config.pinctl)
            | self.pio.ms(rp_pio::SFR_SM0_PINCTRL_PINS_SIDE_COUNT, bit_count as _);

        self.config.execctl =
            self.pio.zf(
                rp_pio::SFR_SM0_EXECCTRL_SIDE_PINDIR,
                self.pio.zf(rp_pio::SFR_SM0_EXECCTRL_SIDESET_ENABLE_BIT, self.config.execctl),
    ...
    pub fn config_set_out_shift(&mut self, shift_right: bool, autopull: bool, pull_threshold: usize) {
        ...
        self.config.shiftctl =
            self.pio.zf(
                rp_pio::SFR_SM0_SHIFTCTRL_OSR_THRESHOLD,
                self.pio.zf(
                    rp_pio::SFR_SM0_SHIFTCTRL_AUTO_PULL,
                    self.pio.zf(rp_pio::SFR_SM0_SHIFTCTRL_OUT_SHIFT_DIR, self.config.shiftctl),
                ),
            ) | self.pio.ms(
                rp_pio::SFR_SM0_SHIFTCTRL_OSR_THRESHOLD,
                if pull_threshold == 32 { 0 } else { pull_threshold as _ },
    ...
    pub fn config_set_in_shift(&mut self, shift_right: bool, autopush: bool, push_threshold: usize) {
        ...
        self.config.shiftctl =
            self.pio.zf(
                rp_pio::SFR_SM0_SHIFTCTRL_ISR_THRESHOLD,
                self.pio.zf(
                    rp_pio::SFR_SM0_SHIFTCTRL_AUTO_PUSH,
                    self.pio.zf(rp_pio::SFR_SM0_SHIFTCTRL_IN_SHIFT_DIR, self.config.shiftctl),
                ),
            ) | self.pio.ms(
                rp_pio::SFR_SM0_SHIFTCTRL_ISR_THRESHOLD,
                if push_threshold == 32 { 0 } else { push_threshold as _ },
    ...
    pub fn config_set_defaults(&mut self) { self.config = SmConfig::default(); }

    pub fn config_set_jmp_pin(&mut self, pin: usize) {
        assert!(pin < 32);
        self.config.execctl = self.pio.zf(rp_pio::SFR_SM0_EXECCTRL_JMP_PIN, self.config.execctl)
    ...
    pub fn clkdiv_from_float(&self, div: f32) -> (u16, u8) {
        ...
        let div_frac = if div_int == 0 { 0u8 } else { ((div - div_int as f32) * (1 << 8) as f32) as u8 };
        (div_int, div_frac)
    }

    pub fn config_set_clkdiv_int_frac(&mut self, div_int: u16, div_frac: u8) {
        assert!(!(div_int == 0 && (div_frac != 0)));
        self.config.clkdiv = self.pio.ms(rp_pio::SFR_SM0_CLKDIV_DIV_INT, div_int as _)
    ...
    pub fn config_set_wrap(&mut self, start: usize, end: usize) {
        self.config.execctl = self.pio.zf(
        ...
        self.config.execctl =
            self.pio.zf(
                ...
                self.pio.zf(
                    ...
                    self.pio.zf(rp_pio::SFR_SM0_EXECCTRL_INLINE_OUT_EN, self.config.execctl),
                ),
    ...
    pub fn config_set_fifo_join(&mut self, join: PioFifoJoin) {
        self.config.shiftctl = self.pio.zf(
            rp_pio::SFR_SM0_SHIFTCTRL_JOIN_RX,
            self.pio.zf(rp_pio::SFR_SM0_SHIFTCTRL_JOIN_TX, self.config.shiftctl),
        ) | match join {
            PioFifoJoin::None => 0,
    ...
    pub fn config_set_mov_status(&mut self, status_sel: MovStatusType, level: usize) {
        self.config.execctl = self.pio.zf(
            ...
            self.pio.zf(rp_pio::SFR_SM0_EXECCTRL_STATUS_N, self.config.execctl),
    ...
    pub fn sm_exec(&mut self, instr: u16) {
        ...
        unsafe {
            self.pio.base().add(rp_pio::SFR_SM0_INSTR.offset() + sm_offset).write_volatile(instr as u32);
        }
    }
    ...
    pub fn sm_set_pindirs_with_mask(&mut self, pindirs: usize, mut pin_mask: usize) {
        ...
        unsafe {
            let pinctrl_saved =
                self.pio.base().add(rp_pio::SFR_SM0_PINCTRL.offset() + sm_offset).read_volatile();
            let exectrl_saved =
                self.pio.base().add(rp_pio::SFR_SM0_EXECCTRL.offset() + sm_offset).read_volatile();
            self.pio.base().add(rp_pio::SFR_SM0_EXECCTRL.offset() + sm_offset).write_volatile(self.pio.zf(
                rp_pio::SFR_SM0_EXECCTRL_OUT_STICKY,
                self.pio.base().add(rp_pio::SFR_SM0_EXECCTRL.offset() + sm_offset).read_volatile(),
            ));
            while pin_mask != 0 {
    ...
    pub fn global_irq0_source_enabled(&mut self, source: PioRawIntSource, enabled: bool) {
        self.pio.wo(
            rp_pio::SFR_IRQ0_INTE,
            if enabled { source as u32 } else { 0 } | self.pio.r(rp_pio::SFR_IRQ0_INTE) & !(source as u32),
        );
    }
    ...
    pub fn global_irq1_source_enabled(&mut self, source: PioRawIntSource, enabled: bool) {
        self.pio.wo(
            rp_pio::SFR_IRQ1_INTE,
            if enabled { source as u32 } else { 0 } | self.pio.r(rp_pio::SFR_IRQ1_INTE) & !(source as u32),
        );
    }
    ...
    pub fn sm_irq0_source_enabled(&mut self, source: PioIntSource, enabled: bool) {
        ...
        self.pio.wo(
            rp_pio::SFR_IRQ0_INTE,
            if enabled { mask } else { 0 } | self.pio.r(rp_pio::SFR_IRQ0_INTE) & !mask,
        );
    }
    ...
    pub fn sm_irq1_source_enabled(&mut self, source: PioIntSource, enabled: bool) {
        ...
        self.pio.wo(
            rp_pio::SFR_IRQ1_INTE,
            if enabled { mask } else { 0 } | self.pio.r(rp_pio::SFR_IRQ1_INTE) & !mask,
        );
    }
    ...
    pub fn sm_set_enabled(&mut self, enabled: bool) {
        if enabled {
            self.pio.rmwf(rp_pio::SFR_CTRL_EN, self.pio.rf(rp_pio::SFR_CTRL_EN) | (self.sm_bitmask()))
        } else {
            self.pio.rmwf(rp_pio::SFR_CTRL_EN, self.pio.rf(rp_pio::SFR_CTRL_EN) & !(self.sm_bitmask()))
        }
    }
    ...
    pub fn sm_set_config(&mut self) {
        ...
        unsafe {
            self.pio
                .base()
                .add(rp_pio::SFR_SM0_CLKDIV.offset() + sm_offset)
                .write_volatile(self.config.clkdiv);
            self.pio
                .base()
                .add(rp_pio::SFR_SM0_EXECCTRL.offset() + sm_offset)
                .write_volatile(self.config.execctl);
            self.pio
                .base()
                .add(rp_pio::SFR_SM0_SHIFTCTRL.offset() + sm_offset)
                .write_volatile(self.config.shiftctl);
            self.pio
                .base()
                .add(rp_pio::SFR_SM0_PINCTRL.offset() + sm_offset)
                .write_volatile(self.config.pinctl);
        }
    }
    ...
    pub fn sm_clear_fifos(&mut self) {
        ...
        unsafe {
            ...
            self.pio
                .base()
                .add(rp_pio::SFR_SM0_SHIFTCTRL.offset() + sm_offset)
                .write_volatile(baseval ^ bitval);
            self.pio.base().add(rp_pio::SFR_SM0_SHIFTCTRL.offset() + sm_offset).write_volatile(baseval);
        }
    }
    ...
    pub fn sm_drain_tx_fifo(&mut self) {
        ...
        let instr = {
            if (unsafe {
                self.pio.base().add(rp_pio::SFR_SM0_SHIFTCTRL.offset() + sm_offset).read_volatile()
            } & self.pio.ms(rp_pio::SFR_SM0_SHIFTCTRL_AUTO_PULL, 1))
                != 0
            {
                // autopull is true
                let mut a = pio::Assembler::<32>::new();
    ...
    pub fn sm_jump_to_wrap_bottom(&mut self) {
        ...
        self.pio.wo(
            rp_pio::SFR_CTRL,
            self.pio.r(rp_pio::SFR_CTRL) | self.pio.ms(rp_pio::SFR_CTRL_EN, self.sm_bitmask()),
        );
    }
    ...
    pub fn gpio_set_outover(&mut self, pin: usize, value: bool) {
        self.pio.wo(
            rp_pio::SFR_IO_O_INV,
            (if value { 1 } else { 0 }) << pin | (self.pio.r(rp_pio::SFR_IO_O_INV) & !(1 << pin)),
        );
    }
    ...
    pub fn gpio_set_oeover(&mut self, pin: usize, value: bool) {
        self.pio.wo(
            rp_pio::SFR_IO_OE_INV,
            (if value { 1 } else { 0 }) << pin | (self.pio.r(rp_pio::SFR_IO_OE_INV) & !(1 << pin)),
        );
    }
    ...
    pub fn gpio_set_inover(&mut self, pin: usize, value: bool) {
        self.pio.wo(
            rp_pio::SFR_IO_I_INV,
            (if value { 1 } else { 0 }) << pin | (self.pio.r(rp_pio::SFR_IO_I_INV) & !(1 << pin)),
        );
    }
}
