...
impl Default for XousTargetInner {
    fn default() -> Self { XousTargetInner { step_patch: None } }
}
...
impl XousTarget {
    pub fn patch_stepi(&mut self, tid: Tid) -> Result<(), <XousTarget as Target>::Error> {
        ...
        self.read_register(tid, RiscvRegId::Pc, &mut pc).or(Err("unable to read register"))?;
        let pc = u32::from_le_bytes(pc);

        let mut opcode = [0u8; core::mem::size_of::<u32>()];
        self.read_addrs(pc, &mut opcode, tid).or(Err("unable to read memory"))?;
        ...
        let mut opcode = [0u8; core::mem::size_of::<u32>()];
        self.read_addrs(new_pc, &mut opcode, tid).or(Err("unable to read memory"))?;
        ...
        match existing_opcode_type {
            OpcodeType::Rv16 => self.next_pc_16(new_pc, (existing & 0xffff).try_into().unwrap(), tid)?,
        ...
        let (existing, new_opcode) = match existing_opcode_type {
            ...
            OpcodeType::Rv32 => (Opcode::Opcode32(existing), Opcode::Opcode32(0x0010_0073)), /* c.ebreak c.
                                                                                              * ebreak */
        };

        match new_opcode {
            Opcode::Opcode16(val) => {
                self.write_addrs(new_pc, &val.to_le_bytes(), tid).or(Err("unable to write memory"))?
            }
            Opcode::Opcode32(val) => {
                self.write_addrs(new_pc, &val.to_le_bytes(), tid).or(Err("unable to write memory"))?
            }
        ...
        self.inner.step_patch = Some(PatchedInstruction { pc: new_pc, previous: existing });
    ...
    fn next_pc_16(&mut self, pc: u32, opcode: u16, tid: Tid) -> Result<u32, <XousTarget as Target>::Error> {
        ...
        if opcode & 0b1110_00000_1111111 == 0b1000_00000_0000010 {
            ...
            if opcode & 0b001_00000000000_00 == 0b001_00000000000_00 {
                let target = if rs1_val != 0 { pc.wrapping_add(imm) } else { pc + 2 };
                Ok(target)
            } else {
                let target = if rs1_val == 0 { pc.wrapping_add(imm) } else { pc + 2 };
                Ok(target)
    ...
    fn next_pc_32(&mut self, pc: u32, opcode: u32, tid: Tid) -> Result<u32, <XousTarget as Target>::Error> {
        ...
        else if opcode & 0b000000000000_00000_111_00000_1111111 == 0b000000000000_00000_000_00000_1100111 {
