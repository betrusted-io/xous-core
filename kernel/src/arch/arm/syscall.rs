...
pub fn resume(supervisor: bool, thread: &Thread) -> ! { resume_inner(supervisor, thread, true) }

pub fn resume_no_irqs(supervisor: bool, thread: &Thread) -> ! { resume_inner(supervisor, thread, false) }
...
fn resume_inner(supervisor: bool, thread: &Thread, enable_irqs: bool) -> ! {
    ...
    klog!(
        "resume: setting sp={:08x}, res_addr={:08x}, lr={:08x} | privileged: {:?} | irqs: {:?} | ret: {:08x}",
        thread.sp,
        thread.resume_addr,
        thread.lr,
        supervisor,
