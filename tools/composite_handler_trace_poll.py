...
"""Poll composite_handler entry/exit + IRQ try_lock counters over EP0 (no UART).
...
Response (little-endian u32 x5):
  composite_handler_entries, composite_handler_exits,
  lock_acquired, lock_contended, seq

Can run concurrently with irq_pending_trace_poll.py (0x42) and
bulk_trb_trace_poll.py (0x43) — independent EP0 vendor requests.
...
Example:
  python3 tools/composite_handler_trace_poll.py -o composite-handler-trace.log
"""
...
def _exit_permission_denied(vid: int, pid: int) -> None:
    ...
    cmd = f"sudo python3 tools/composite_handler_trace_poll.py {args}".rstrip()
    print(
...
def read_stats(dev, timeout_ms: int) -> tuple[int, int, int, int, int]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Poll composite_handler entry/exit + try_lock via EP0 vendor IN"
    )
    ...
    parser.add_argument("-o", "--output", default="composite-handler-trace.log")
    ...
    print(
        ...
        "STALL after reflash = bug (wrong firmware or 0x44 not wired). "
        "Safe alongside irq_pending_trace_poll.py (0x42) and bulk_trb_trace_poll.py (0x43).",
        flush=True,
    )
    try:
    ...
    with open(args.output, "a", buffering=1) as log:
        ...
        while True:
            ...
            try:
                entries, exits, lock_ok, lock_busy, seq = read_stats(dev, args.timeout_ms)
                saw_success = True
                flags: list[str] = []
                ...
                inflight = int(entries) - int(exits)
                if inflight != 0:
                    flags.append(f"INFLIGHT={inflight}")

                last_seq = seq

                flag_s = (" " + " ".join(flags)) if flags else ""
                line = (
                    f"{ts} entries={entries} exits={exits} "
                    f"lock_acquired={lock_ok} lock_contended={lock_busy} "
                    f"seq={seq}{flag_s}"
                )
