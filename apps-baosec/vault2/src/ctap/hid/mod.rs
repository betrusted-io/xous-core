...
mod test {
    ...
    fn test_split_assemble() {
        ...
        for payload_len in 0..7609 {
            ...
            for packet in HidPacketIterator::new(message.clone()).unwrap() {
                match assembler.parse_packet(&mut env, &packet, Instant::new(0)) {
    ...
    fn test_spurious_continuation_packet() {
        ...
        assert_eq!(
            ctap_hid.parse_packet(&mut env, &packet, Instant::new(0)),
            None
        );
    }
    ...
    fn test_command_init_for_sync() {
        ...
        assert_eq!(
            ctap_hid.parse_packet(&mut env, &packet1, Instant::new(0)),
            None
        );
        assert_eq!(
            ctap_hid.parse_packet(&mut env, &packet2, Instant::new(0)),
            Some(Message {
                cid,
                cmd: CtapHidCommand::Init,
                payload: vec![
    ...
    fn test_command_ping() {
        ...
        assert_eq!(
            ctap_hid.parse_packet(&mut env, &ping_packet, Instant::new(0)),
            Some(Message {
                cid,
    ...
    fn test_command_cancel() {
        ...
        let response = ctap_hid.parse_packet(&mut env, &cancel_packet, Instant::new(0));
