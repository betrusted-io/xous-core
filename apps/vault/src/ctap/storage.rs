...
pub fn deserialize_credential(data: &[u8]) -> Option<PublicKeyCredentialSource> {
    let cbor = super::cbor_read(data).ok()?;
    cbor.try_into().ok()
}

/// Serializes a credential to storage representation.
pub fn serialize_credential(credential: PublicKeyCredentialSource) -> Result<Vec<u8>, Ctap2StatusCode> {
