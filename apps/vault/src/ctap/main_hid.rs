...
impl MainHid {
    ...
    pub fn should_wink(&self, now: Instant) -> bool {
        self.wink_permission.is_granted(now)
    }

    /// Updates the timeout for the wink permission.
    pub fn update_wink_timeout(&mut self, now: Instant) {
...
mod test {
    ...
    fn test_wink() {
        ...
        let mut ctap_state = CtapState::new(&mut env, Instant::new(0));
        let (mut main_hid, cid) = new_initialized();
        assert!(!main_hid.should_wink(Instant::new(0)));
        ...
        assert_eq!(response.next(), None);
        assert!(main_hid.should_wink(Instant::new(0)));
        assert!(!main_hid.should_wink(Instant::new(1) + MainHid::WINK_TIMEOUT_DURATION));
    }
}
