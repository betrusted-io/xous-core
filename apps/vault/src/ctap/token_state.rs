...
const INITIAL_USAGE_TIME_LIMIT: Duration = Duration::from_millis(30000);
...
impl PinUvAuthTokenState {
    ...
    pub fn begin_using_pin_uv_auth_token(&mut self, now: Instant) {
    ...
    pub fn pin_uv_auth_token_usage_timer_observer(&mut self, now: Instant) {
        if !self.in_use {
            return;
        }
