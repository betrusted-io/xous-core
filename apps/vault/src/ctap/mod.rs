...
use ctap_crypto::hmac::hmac_256;
use ctap_crypto::sha256::Sha256;
use ctap_crypto::{ecdsa, Hash256};
...
pub const TOUCH_TIMEOUT_MS: u64 = 30000;
#[cfg(feature = "with_ctap1")]
...
// Needs more timing tolerance for Xous automated test bench, expanded to 20 seconds.
#[cfg(feature="xous")]
...
#[cfg(feature="xous")]
fn check_user_presence(env: &mut impl Env, channel: Channel, reason: Option<String>) -> Result<(), Ctap2StatusCode> {
    env.user_presence().check_init();

    let cid = match channel {
        Channel::MainHid(cid) => cid,
    };
    let result = env.user_presence().wait_with_timeout(
        TOUCH_TIMEOUT,
        reason,
        cid,
    );
...
impl StatefulPermission {
    ...
    pub fn new_reset(now: Instant) -> StatefulPermission {
        StatefulPermission {
    ...
    pub fn clear_timer(&mut self, now: Instant) {
...
impl CtapState {
    ...
    #[cfg(feature = "with_ctap1")]
    pub fn u2f_grant_user_presence(&mut self, now: Instant) {
...
mod test {
    ...
    fn test_get_info() {
        ...
        let info_reponse =
            ctap_state.process_command(&mut env, &[0x04], DUMMY_CHANNEL, Instant::new(0));
    ...
    fn test_get_info_no_pin_protocol_v1() {
        ...
        let ctap_state = CtapState::new(&mut env, Instant::new(0));
    ...
    fn test_helper_process_make_credential_with_pin_and_uv(
        ...
        let mut env = TestEnv::new();
        let key_agreement_key = ctap_crypto::ecdh::SecKey::gensk(env.rng());
    ...
    fn test_helper_process_get_assertion_hmac_secret(pin_uv_auth_protocol: PinUvAuthProtocol) {
        ...
        let key_agreement_response =
            ctap_state
                .client_pin
                .process_command(&mut env, client_pin_params, Instant::new(0));
    ...
    fn test_process_get_next_assertion_not_allowed() {
        ...
        ctap_state.process_command(&mut env, &command_cbor, DUMMY_CHANNEL, Instant::new(0));
    ...
    fn test_process_reset() {
        ...
        let reset_reponse =
            ctap_state.process_command(&mut env, &[0x07], DUMMY_CHANNEL, Instant::new(0));
    ...
    fn test_process_reset_not_first() {
        ...
        ctap_state.process_command(&mut env, &[0x08], DUMMY_CHANNEL, Instant::new(0));
    ...
    fn test_process_unknown_command() {
        ...
        let reponse =
            ctap_state.process_command(&mut env, &[0xDF], DUMMY_CHANNEL, Instant::new(0));
    ...
    fn test_permission_timeout() {
        ...
        let get_next_assertion_response = ctap_state.process_parsed_command(
            ...
            DUMMY_CHANNEL,
            Instant::new(0) + STATEFUL_COMMAND_TIMEOUT_DURATION - Duration::from_millis(1),
        );
    ...
    fn test_reset_timeout() {
        ...
        let response = ctap_state.process_parsed_command(
            ...
            DUMMY_CHANNEL,
            Instant::new(0) + RESET_TIMEOUT_DURATION + Duration::from_millis(1),
        );
    ...
    fn test_check_user_presence() {
        ...
        let mut env = TestEnv::new();
        let response = check_user_presence(&mut env, DUMMY_CHANNEL, None);
