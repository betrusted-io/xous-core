    ...
    pub fn process_command(
        ...
        match command {
            U2fCommand::Register {
                ...
                #[cfg(feature="xous")]
                let regstr = String::from(t!("vault.u2f.register", locales::LANG));
                #[cfg(feature="xous")]
            ...
            U2fCommand::Authenticate {
                ...
                #[cfg(feature="xous")]
                let authstr = String::from(t!("vault.u2f.authenticate", locales::LANG));
                // The order is important due to side effects of checking user presence.
                #[cfg(feature="xous")]
                if flags == Ctap1Flags::EnforceUpAndSign
    ...
    fn process_authenticate(
        ...
        if let Some(credential_source) = credential_source {
            ...
            let mut signature_data = ctap_state
                ...
                .map_err(|_| Ctap1StatusCode::SW_WRONG_DATA)?;
            signature_data.extend(&challenge);
            let signature = ecdsa_key.sign_rfc6979::<ctap_crypto::sha256::Sha256>(&signature_data);
...
mod test {
    ...
    fn test_process_allowed() {
        ...
        ctap_state.u2f_up_state.consume_up(Instant::new(0));
        ctap_state.u2f_up_state.grant_up(Instant::new(0));
        let response =
            Ctap1Command::process_command(&mut env, &message, &mut ctap_state, Instant::new(0));
    ...
    fn test_process_register_without_up() {
        ...
        ctap_state.u2f_up_state.consume_up(Instant::new(0));
        ctap_state.u2f_up_state.grant_up(Instant::new(0));
        let timeout_clock_value: Instant =
            Instant::new((30001 * TEST_CLOCK_FREQUENCY_HZ as u64) / 1000);
        let response =
