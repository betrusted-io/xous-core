...
use cbor::{cbor_map_options, destructure_cbor_map};
...
fn decrypt_legacy_credential_id(
    ...
    let aes_enc_key = ctap_crypto::aes256::EncryptionKey::new(&env.key_store().key_handle_encryption()?);
...
mod test {
    ...
    fn legacy_encrypt_to_credential_id(
        ...
        let aes_enc_key =
            ctap_crypto::aes256::EncryptionKey::new(&env.key_store().key_handle_encryption()?);
