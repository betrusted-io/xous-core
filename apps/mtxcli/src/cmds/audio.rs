...
impl Audio {
    pub fn new(xns: &xous_names::XousNames) -> Self {
        let callback_conn = xns.request_connection_blocking(crate::SERVER_NAME_MTXCLI).unwrap();
        Audio {
            callback_id: None,
            callback_conn,
            framecount: 0,
...
impl<'a> ShellCmdApi<'a> for Audio {
    ...
    fn process(&mut self, args: String::<1024>, env: &mut CommonEnv) -> Result<Option<String::<1024>>, xous::Error> {
        ...
        let helpstring = t!("mtxcli.audio.help", xous::LANG);
        ...
        if let Some(sub_cmd) = tokens.next() {
            match sub_cmd {
                "tone" => {
                    ...
                    write!(ret, "{}", t!("mtxcli.audio.start", xous::LANG)).unwrap();
                }
                _ => {
        ...
        match &msg.body {
            Message::Scalar(xous::ScalarMessage{id: _, arg1: free_play, arg2: _avail_rec, arg3: _, arg4: routing_id}) => {
                if *routing_id == codec::AUDIO_CB_ROUTING_ID {
                    ...
                    write!(ret, "{} {} {}.",
                        t!("mtxcli.audio.completion_a", xous::LANG),
                        self.framecount,
                        t!("mtxcli.audio.completion_b", xous::LANG),
                    ).unwrap();
                    self.framecount = 0;
                    self.play_sample = 0.0;
