...
pub fn irq_setup() {
    ...
    let mut legacy_int = CSR::new(HW_LEGACY_INT_BASE as *mut u32);
    legacy_int.wo(utra::legacy_int::MACH_MASK, 0); // first make sure everything is disabled, so we aren't OR'ing in garbage
    unsafe {
        mstatus::set_mie();
    }
...
pub fn enable_irq(irq_no: usize) {
    ...
    legacy_int.wo(utra::legacy_int::MACH_MASK, legacy_int.r(utra::legacy_int::MACH_MASK) | (1 << irq_no));
    // must enable external interrupts on the CPU for any of the above to matter
    unsafe { mie::set_mext() };
}
...
pub unsafe extern "C" fn _start_trap() -> ! {
    loop {
        ...
        pub unsafe extern "C" fn _start_trap_aligned() {
            ...
            core::arch::asm!(
                "csrw        mscratch, sp",
                "li          sp, 0x4000C000", // crate::platform::SCRATCH_PAGE - has to be hard-coded
                "sw       x1, 0*4(sp)",
                // Skip SP for now
                "sw       x3, 2*4(sp)",
                "sw       x4, 3*4(sp)",
                ...
                "li          sp, 0x4000E000", // last page of RAM, grows down
...
pub extern "C" fn trap_handler(
    ...
    if cause.bits() == 2 {
        ...
        let legacy_int = CSR::new(HW_LEGACY_INT_BASE as *mut u32);
        let irqs_pending = legacy_int.r(utra::legacy_int::MACH_PENDING);
        ...
        } else if (irqs_pending & (1 << utra::uart::UART_IRQ)) != 0 {
