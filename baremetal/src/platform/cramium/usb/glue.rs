...
use crate::usb;
use crate::usb::SliceCursor;
use crate::usb::sha512_digest::Sha512Prehash;
...
// exclusive of the signature block offset. Vendored in from the cramium platform definition.
pub const KERNEL_BASE_ADDR: usize = 0x6_0000;

const SIGBLOCK_SIZE: usize = 0x1000;
                    ...
                    for entry in dir {
                        ...
                        if let Some(file_name) = entry.path().file_name() {
                            if file_name.to_ascii_uppercase() == KERNEL_NAME {
                                ...
                                let pubkey =
                                    VerifyingKey::from_bytes(&DEV_PUBKEY).expect("public key was not valid");
                                crate::println!("pubkey as reconstituted: {:x?}", pubkey);

                                let k_start = sector_offset as usize * 512;
                                let sig_region =
                                    &disk_access[k_start..k_start + core::mem::size_of::<SignatureInFlash>()];
                                ...
                                let ssh: &SwapSourceHeader =
                                    (swap_image.as_ptr() as *const SwapSourceHeader).as_ref().unwrap();
                                        ...
                                        == 0x73776170
                                {
                                    ...
                                    for (page_no, chunk) in swap_image.chunks(FLASH_PAGE_LEN).enumerate() {
                                        ...
                                        if ((page_no * FLASH_PAGE_LEN) % 0x1000) == 0 {
                                            flash_spim.flash_erase_sector((page_no * FLASH_PAGE_LEN) as u32);
                                        }

                                        flash_spim
                                            .mem_flash_write_page((page_no * FLASH_PAGE_LEN) as u32, &buf);
