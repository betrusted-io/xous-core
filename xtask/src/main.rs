...
// This was autogenerated by xtask/build.rs.

pub(crate) fn get_version() -> crate::api::VersionString {
    let mut v = crate::api::VersionString {
        version: xous_ipc::String::new()
    };
    // NOTE: the semver is only updated if you can somehow trigger build.rs on xtask to re-run. AFAIK, it basically never re-runs,
    // because it will only rerun if something *inside xtask* changes (not the rest of the code).
    // so if you want to pick up the "correct" SEMVER, you have to do a cargo clean.
    v.version.append(SEMVER).ok();
    v.version.append("\n").ok();
    // This timestamp *will* be correct because we are actually forcing it to be rebuilt every build.
