...
Updated: 2026-08-18 — Persona A + CCID bulk-only (6/8) + cumulative EpBudgetLedger.
-->
...
| `tools/ccid_hil/ccid_usb.py` | `assert_persona_a_composite` (6 non-EP0, ≥2 HID, ≥1 CCID, 0 CDC) | Catch wrong composite without hardcoded IF indices |
...
| `xtask baosec-ccid` / `dabao-ccid` / `ccid-hil` | CCID(2)+FIDO(2)+NKRO(2) | **6** | OK (2 spare) | Persona A; interrupt IN omitted |
| REJECTED: CCID+HID+CDC | 2+2+2+3 | **9** | OVERFLOW | historical was 10 with CCID interrupt IN |
| REJECTED: +prov CDC | 9+3 | **12** | OVERFLOW | path removed |
...
2. Stock `baosec` is 7/8; CCID images are 6/8 — any new USB class still needs an exclusion.
