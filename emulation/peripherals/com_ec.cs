...
namespace Antmicro.Renode.Peripherals.SPI
{
    ...
    {
        public BetrustedEcRam(Machine machine, BetrustedEcSpi spi, long size)
        {
        ...
        {
            return writeFifo.TryDequeue(out value);
        }
        ...
        public void WriteDoubleWord(long address, uint value)
        {
            if (address == 0)
            {
                machine.Log(LogLevel.Error, "EC sending word: {0:X4}", value);
    ...
    public class BetrustedEcSpi : ISPIPeripheral, IDoubleWordPeripheral, IKnownSize, IProvidesRegisterCollection<DoubleWordRegisterCollection>, IGPIOSender
    {

        public BetrustedEcSpi(Machine machine, uint memAddr, uint memSize)
        {
            HOLD = new GPIO();
            ...
            Registers.ComControl.Define32(this)
                ...
                .WithFlag(1, FieldMode.Read | FieldMode.Write, name: "HOST_INT")
                .WithFlag(2, FieldMode.Write, writeCallback: (_, val) =>
                {
                    if (!val)
                        return;
                    machine.Log(LogLevel.Error, "Resetting FIFO due to RESET bit being set");
                    Reset();
                }, name: "RESET")
            ;
            ...
            Registers.ComStatus.Define32(this)
                ...
                .WithFlag(16, FieldMode.Read, valueProviderCallback: (_) => EcRam.WriteFifoCount() < 1280, name: "TX_AVAIL")
                ...
                .WithFlag(31, FieldMode.Read, valueProviderCallback: (_) => !EcRam.WriteFifoHasData(), name: "TX_UNDER")
            ;
            Reset();
        }
        ...
        public byte Transmit(byte value)
        {
            if (IsFirstByte)
            {
                ...
                FirstRxByte = (uint)value;
                IsFirstByte = false;
                return (byte)(TxValue);
            }
            else
            {
                uint secondRxByte = (uint)value;
                this.Log(LogLevel.Info, "EC received:{0:X2}{1:X2} sent:{2:X2}{3:X2}", secondRxByte, FirstRxByte, (TxValue >> 8), (TxValue & 0xff));
                IsFirstByte = true;
                uint receivedWord = FirstRxByte | (secondRxByte << 8);
                EcRam.WriteNextWord(receivedWord);
                this.Log(LogLevel.Info, "EcRam.ReadFifo now contains {0} items", EcRam.ReadFifoCount());
                return (byte)(TxValue >> 8);
            }
        }
